...
Loaded post-FT checkpoint $\to$ \texttt{torch.sparse.\allowbreak to\_sparse\_\allowbreak semi\_structured} on every convertible Linear layer; for ResNet Conv2d rows, exact Conv2d lowering to im2col+\texttt{Linear} before the same 2:4 conversion. &
A40 batch-128 native Linear 2:4 switches of $1.36\times$ ViT-B+ToMe, $1.37\times$ ViT-L+ToMe, $1.36/1.38/1.33\times$ for DeiT-B/S/T, and $1.26\times$ for ConvNeXtV2-B; ResNet exact sparse-im2col switches of $1.50$--$1.70\times$ versus dense im2col. \\
\bottomrule
\end{tabular}
\end{table}
...
The deployability audit is read-only: it loads the saved post-FT checkpoint, changes only the in-memory execution endpoint, and benchmarks dense and sparse endpoints with the validation accuracy fixed by the checkpoint.  On an A40 at batch \(128\), the native PyTorch/NVIDIA 2:4 Linear backend gives \(1.36\times\) for ViT-B/16+ToMe, \(1.37\times\) for ViT-L/16+ToMe, \(1.36/1.38/1.33\times\) for DeiT-B/S/T+ToMe, and \(1.26\times\) for ConvNeXtV2-Base.  These are same-checkpoint backend switches.  The older A100 no-ToMe ViT-B benchmark remains a separate endpoint comparison for the un-tokened graph.
