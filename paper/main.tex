...
On ImageNet-1k after three distillation epochs, ViT-B/16 \(2{:}4{+}\)ToMe reaches \(83.41\%\) top-1 (\(-1.70\) pp from dense) at \(59.81\%\) sparse-execution MAC reduction, with a \(1.36\times\) measured A40 native-\(2{:}4\) backend speedup for the same checkpoint and ToMe graph, and a \(2.705\times\) measured A100 no-ToMe dense-to-\(2{:}4\) endpoint.  At structured sparsity, ViT-B/16 \(6{:}12\) reaches \(83.74\%\), ViT-L/16 \(8{:}16\) dense+permutation reaches \(85.33\%\) (\(-0.51\) pp), and ConvNeXtV2-Base \(12{:}16\) reaches \(86.35\%\) (\(-0.37\) pp).  The wider \(6{:}12\), \(8{:}16\), and \(12{:}16\) rows are accuracy-focused \(N{:}M\) projections rather than native 2:4 sparse-kernel throughput claims.\footnote{Code:
...
baselines on ImageNet-1k. The table includes ViT-B/16 \(2{:}4{+}\)ToMe at \(83.41\%\) top-1 with \(1.36\times\) A40 same-checkpoint native-\(2{:}4\) speedup and \(2.705\times\) A100 no-ToMe dense-to-\(2{:}4\) endpoint speedup; ViT-B/16 \(6{:}12\) at \(83.74\%\); ViT-L/16 \(8{:}16\) dense+perm at \(85.33\%\); ConvNeXtV2-Base \(12{:}16\) at \(86.35\%\); and Hybrid Magnitude--SER at \(83.37\%\) on ViT-B/16.
...
Loaded post-FT checkpoint $\to$ \texttt{torch.sparse.\allowbreak to\_sparse\_\allowbreak semi\_structured} on every convertible Linear layer; for ResNet Conv2d rows, exact Conv2d lowering to im2col+\texttt{Linear} before the same 2:4 conversion, plus a separate 1$\times$1-only hybrid control. &
A40 batch-128 native Linear 2:4 switches of $1.36\times$ ViT-B+ToMe, $1.37\times$ ViT-L+ToMe, $1.36/1.38/1.33\times$ for DeiT-B/S/T, and $1.26\times$ for ConvNeXtV2-B; batch-sweep bests reach $1.38$--$1.39\times$ on the larger Transformer rows and $1.29\times$ on ConvNeXtV2-B. ResNet exact sparse-im2col gives $1.50$--$1.70\times$ versus dense im2col; the 1$\times$1-only hybrid is $0.78$--$0.81\times$ versus cuDNN Conv2d, so it is a control rather than a speedup claim. \\
\bottomrule
\end{tabular}
\end{table}
...
For the ViT-B/16 \(2{:}4{+}\)ToMe row, measured A40 throughput at batch \(128\) changes from \(1246.1\) to \(1700.4\) images/s, a \(1.365\!\times\) ratio with ToMe held constant on both endpoints.  The magnitude-2:4+ToMe baseline reports \(82.92\%\), and the CAST row reports \(83.41\%\) at the same nominal sparsity, a \(+0.49\) pp difference.  The ToMe-only ViT-B/16 (AugReg) reference numbers in \cite{bolya2023tome} Table 8 are \(83.94\%\) at \(r=8\) and \(82.86\%\) at \(r=12\); the magnitude-2:4+ToMe-r=8 baseline at \(82.92\%\) is \(1.02\) pp below the dense-kernel ToMe-r=8 reference.

\paragraph{Hardware-speedup measurements.}
The deployability audit is read-only: it loads the saved post-FT checkpoint, changes only the in-memory execution endpoint, and benchmarks dense and sparse endpoints with the validation accuracy fixed by the checkpoint.  On an A40 at batch \(128\), the native PyTorch/NVIDIA 2:4 Linear backend gives \(1.36\times\) for ViT-B/16+ToMe, \(1.37\times\) for ViT-L/16+ToMe, \(1.36/1.38/1.33\times\) for DeiT-B/S/T+ToMe, and \(1.26\times\) for ConvNeXtV2-Base.  A batch sweep over \(64,128,256\) gives best observed same-checkpoint A40 ratios of \(1.388\times\) for ViT-B/16+ToMe, \(1.394\times\) for ViT-L/16+ToMe, \(1.384/1.376/1.330\times\) for DeiT-B/S/T+ToMe, and \(1.295\times\) for ConvNeXtV2-Base.  These are backend switches, not new validation runs.  The older A100 no-ToMe ViT-B benchmark remains a separate endpoint comparison for the un-tokened graph.
...
\medskip
The measured \(2.705\!\times\) ViT-B/16 no-ToMe endpoint and the A40 same-checkpoint backend switches are empirical benchmarks for the listed implementation, precision, batch size, and PyTorch sparse-kernel path, not theoretical limits. NVIDIA Sparse Tensor Cores accelerate eligible \(2{:}4\) GEMMs by skipping half of the structured operand; realized model throughput also depends on eligible-layer share, layout, precision, framework kernels, and non-GEMM overhead.

\paragraph{ResNet CAST-conv+perm vs.\ dense baseline (full family).}
The four CAST-conv+perm rows of Table~\ref{tab:param_to_flop_followup} reach $75.67\%$, $78.00\%$, $80.59\%$, and $81.33\%$ post-FT on ResNet50/50d/101d/152d respectively, against \texttt{timm} dense baselines $76.13\%$, $80.55\%$, $82.26\%$, and $82.86\%$. The per-row gaps from dense are $-0.46$, $-2.55$, $-1.67$, and $-1.53$~pp; the arithmetic mean is $-1.55$~pp at $\sim\!50\%$ MAC reduction. A read-only A40 audit lowers the exact flattened-\(2{:}4\) Conv2d weights to im2col+\texttt{Linear} and converts the lowered weights to the native 2:4 sparse Linear backend. This gives sparse-im2col speedups of \(1.70\), \(1.50\), \(1.57\), and \(1.62\times\) over dense im2col for ResNet50/50d/101d/152d, respectively. A second audit lowers only the exact 1$\times$1 bottleneck convolutions to Linear and leaves the rest of the network as Conv2d; it converts 36/36/70/104 1$\times$1 layers and gives \(1.087\), \(1.088\), \(1.081\), and \(1.082\times\) over the corresponding dense 1$\times$1-linear hybrid, but only \(0.813\), \(0.802\), \(0.788\), and \(0.782\times\) of native cuDNN Conv2d throughput. Thus the ResNet rows document exact sparse-GEMM deployability, not faster cuDNN or TensorRT Conv2d inference for the current checkpoints. Per-architecture interpretation, including the role of the importance-balanced Cin permutation and the per-architecture cert-audit, is in \methref{sec:cast-2e-pipeline}.
...
Only rows using a \(2{:}4\) mask on convertible Linear layers are native PyTorch/NVIDIA sparse-Tensor-Core speedup claims under the tested path. The \(6{:}12\), \(8{:}16\), and \(12{:}16\) rows impose structured sparsity and reduce arithmetic MAC counts in the model accounting, but they do not map to the native \(2{:}4\) Sparse Tensor Core path used for the measured Linear speedups. They are therefore accuracy and MAC-accounting rows, not sparse-kernel throughput claims. The measured deployable Linear rows are ViT-B/16, ViT-L/16, DeiT-B/S/T, and ConvNeXtV2-Base in Table~\ref{tab:param_to_flop_followup}. The ResNet rows have a separate exact im2col+\(2{:}4\) sparse-GEMM audit, but their checkpoints do not satisfy the TensorRT-style input-channel \(2{:}4\) Conv2d audit documented in the companion methodology paper; the im2col audit is slower than cuDNN Conv2d end-to-end.
