...
\caption{\(\Delta\)Top-1 relative to dense at approximately \(50\%\) sparse-execution MAC reduction vs.\ dense parameter count. Points are rows of Table~\ref{tab:param_to_flop_followup}; the dashed line is an OLS fit in \(\log_{10}\) parameters.}
...
\begin{table}[ht]
\centering
\caption{CAST \(k{:}n\) structured-projection results for parameter-to-compute conversion. Entries are post-FT ImageNet-1k top-1 under the source checkpoints and sparse-execution MAC assumptions described in the surrounding text.}
...
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize \textsuperscript{\dag}ViT-L/16 reduction is estimated from the analytical 22.81\% ToMe reduction plus the 50\% 2:4 Linear share. \textsuperscript{\ddag}\textbf{CAST-conv+perm} uses the importance-balanced Cin permutation alignment of the companion methodology paper. \textsuperscript{\S}Rows marked \(\S\) are wider-pattern or lower-sparsity accuracy probes; they do not claim native 2:4 sparse-tensor-core speedup.}
\end{table}
...
\caption{Selected ImageNet-1k pruning and structured-sparsity results. Non-bold rows are cited results or author-run reimplementations identified in the Notes column; bold rows are this paper's reported results. ``FT ep.'' is the reported post-pruning fine-tuning budget.}
