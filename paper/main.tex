...
Compressing Vision Transformers has been studied along several complementary axes. Token-level methods reduce sequence length: dynamic token pruning \cite{rao2021dynamicvit, liang2022evit, yin2022avit, kong2022spvit}, token merging \cite{bolya2023tome}, and head-/block-level decisions \cite{kuznedelev2023ovit, chen2021svite, zhang2022minivit, zheng2022savit, tai2022spartan, yang2023nvit, fang2024isomorphic, yu2023xpruner, zhang2024dcvit}. These methods reduce wall-clock FLOPs but do not directly target the parameter count of the dense projections. Weight-pruning methods, by contrast, sparsify the dense affine maps themselves; recent unstructured pruning at scale uses second-order signals (SparseGPT \cite{frantar2023sparsegpt}, Wanda \cite{sun2024wanda}) or the heavy-tailed self-regularization framework (AlphaPruning \cite{lu2024alphapruning, martin2020heavy}). Hardware-friendly structured sparsity is dominated by NVIDIA 2:4 sparse Tensor Cores \cite{mishra2021accelerating}, whose deployable speedup motivated CAST-style methods \cite{sun2025mdp} that produce a 2:4 mask after a single short distillation phase. Our work targets both axes at once: at high unstructured sparsity the Hybrid Magnitude--SER protocol of Section~\ref{sec:pruning_VITs} preserves accuracy across 17 trained checkpoints (Table~\ref{tab:multi_arch_hybrid}), and the cert-aware $k{:}n$ projection of Section~\ref{sec:pruning_VITs} (with full methodology in \methref{cast-2e-pipeline}) produces deployable 2:4 masks that achieve a $2.7\times$ A100 throughput improvement at $\le\!0.5$~pp ImageNet top-1 cost after only three epochs of distillation. The same projection delivers theoretical $50\%$ MAC reductions across ResNet50/50d/101d/152d and ConvNeXtV2-Base with post-FT top-1 within $0.6$~pp of the dense baseline.
...
Vision Transformers are a natural testbed for the Marchenko--Pastur diagnostics developed above: most of the parameters in a ViT lie in dense attention and MLP projection matrices \cite{dosovitskiy2021image,vaswani2017attention}.  The numerical evidence motivates separating the methods into four increasingly practical protocols.  Compact definitions are given here; the full operational descriptions, all hyperparameters, and the sweeps used to select them are in Appendix~\methref{app:splus_budget}.
\begin{enumerate}[leftmargin=2em]
\item \textbf{Global magnitude pruning} (\emph{Classical magnitude}), the standard unstructured baseline: at sparsity \(s\), the \(s\)-fraction of weights with smallest absolute value across the prunable layers is set to zero, followed by one matched fine-tuning epoch.  Used as the reference column ``Classical magnitude'' in Tables~\ref{tab:vitb_current_methods} and~\ref{tab:multi_arch_hybrid}; full schedule in Appendix~\methref{subsec:splus_method}.
\item \textbf{Classical RMT pruning} (\emph{Classical RMT}), the cycle-based RMT method retained as a diagnostic baseline: fit a per-layer Marchenko--Pastur edge \(\sigma_+(\ell)\), treat sub-edge singular components as bulk-like, sparsify small singular-vector coordinates, and prune the resulting coefficients directly.  Defined in detail in Appendix~\methref{subsec:classical_rmt}.
\item \textbf{Spectral Edge Budgeting} (SEB), a no-fine-tuning method that uses the fitted MP upper edge \(\sigma_+(\ell)\), together with the Hill exponent \(\alpha_\ell\) at very low sparsity, to allocate a layerwise magnitude-pruning budget.  Hyperparameters \((\beta,s_{\mathrm{d}},p)\), the layer-type extension, and the sparsity-regime selection rule are defined in Appendix~\methref{subsec:splus_method}; the SEB-vs-magnitude comparison is Table~\methref{tab:final_comparison} (mask-only) and Table~\methref{tab:final_comparison_10k} (full validation).
\item \textbf{Spectral Edge Reallocation} (SER), the main RMT prune--restore method: prune beyond the target, form a reservoir of removed weights, reinsert an RMT-ranked budget, and run a short frozen-mask fine-tuning phase.  Defined in Appendix~\methref{subsec:ser_method}.  The \emph{Hybrid Magnitude--SER} protocol used in Tables~\ref{tab:vitb_current_methods} and~\ref{tab:multi_arch_hybrid} uses exact global magnitude pruning up to \(s=0.20\) and then switches to SER for \(s\ge0.25\); see Appendix~\methref{subsec:hybrid_ser_method}.
\end{enumerate}

\subsection{Protocols and RMT parameter selection}
\label{pruning_strategy}

All reported ViT-B/16 numbers use the same ImageNet-1k validation set \cite{5206848} and the same checkpoint, \path{vit_base_patch16_224.augreg2_in21k_ft_in1k}, unless explicitly stated otherwise.  For each target layer \(W_\ell\), we fit the MP bulk of \(W_\ell^\top W_\ell/N\) using the BEMA procedure of Appendix~\methref{numerics_updated}.  The resulting layer bank stores \(\sigma_+(\ell)\), MP-fit error, bulk fraction, and the Hill exponent \(\alpha_\ell\).  The pruning rules do not refit these quantities during a run.

The SEB parameters were selected by the sweep in Appendix~\methref{app:splus_budget}: first a signal comparison, then decay-shape and layer-type sweeps, and finally full-validation evaluation of the selected configuration.  The main empirical lesson of that appendix is that the MP edge is weak at very low sparsity but becomes highly informative once magnitude pruning approaches the accuracy cliff.  Consequently, the hybrid protocol intentionally uses exact magnitude pruning through \(20\%\) sparsity and activates RMT-guided reallocation only after the regime where magnitude is already near-optimal.

\subsection{ViT-B/16 results}
\label{subsec:vitb_current_results}

Table~\ref{tab:vitb_current_methods} summarizes the fine-tuned ViT-B/16 evidence.  The no-fine-tuning mask-only comparison is separated into Appendix~\methref{subsec:final_comparison_10k}; the main table compares methods under matched post-pruning validation with a short frozen-mask fine-tuning pass where applicable.

\begin{table}[ht]
\centering
\caption{Fine-tuned ViT-B/16 pruning comparison on ImageNet-1k validation.  Entries are top-1 accuracy (\%) at sparsity \(s\) (fraction of weights set to zero in the prunable layers).  Classical magnitude (Appendix~\methref{subsec:splus_method}) and Classical RMT (Appendix~\methref{subsec:classical_rmt}) use the matched one-cycle fine-tuning protocol.  SER (Appendix~\methref{subsec:ser_method}) uses the prune--restore procedure with a short frozen-mask fine-tuning phase.  Hybrid Magnitude--SER (Appendix~\methref{subsec:hybrid_ser_method}) uses exact global magnitude pruning through \(s=0.20\) and then SER for \(s\ge0.25\).  A dash means that the archived run contains no completed full-validation checkpoint at that sparsity.  Dense baseline: \(85.11\%\).}
...
The hybrid run is motivated by the low-sparsity sweep in Appendix~\methref{app:splus_budget}: at \(s\le0.20\), RMT modulation is unnecessary and sometimes slightly worse than exact magnitude pruning; beyond that point, the spectral budget becomes useful.  The completed hybrid entries retain essentially dense accuracy through \(35\%\) sparsity and remain above both classical baselines through \(70\%\).

\subsection{Interpretation}
\label{subsec:current_numerics_interpretation}

The classical RMT cycle procedure is not the final ViT method --- directly operating on sub-edge singular components is too brittle for modern transformers. The effective use of RMT is \emph{layerwise budget allocation}: MP-like layers (and MLP projections in particular) absorb more pruning, while attention projections need more protection. This matches the deterministic theory in Section~\ref{Main_theory_gen}: once a mask is fixed, the relevant object is the perturbation it induces on the data path, and the fitted MP edge $\sigma_+(\ell)$ acts as a layerwise proxy for where that perturbation is more likely to be admissible. The full layer-by-layer interpretation is in \methref{seb-protocols}.
...
\begin{table}[ht]
\centering
\caption{\textbf{Multi-architecture Hybrid Magnitude--SER sweep on ImageNet-1k validation.}  Entries are post-fine-tuning top-1 accuracy (\%) at sparsity \(s\).  The table contains 17 model rows after omitting the two partial ViT-Small/16 and Swin-Small ledgers.  Unmarked rows use the canonical protocol of Appendix~\methref{subsec:hybrid_ser_method}: exact global magnitude pruning (Appendix~\methref{subsec:splus_method}) through \(s=0.20\) followed by SER (Appendix~\methref{subsec:ser_method}) for \(s\ge0.25\).  Rows marked \(\ddagger\) use the adaptive drop-threshold variant: stage-1 terminates once the post-FT top-1 drops by more than \(0.7\) percentage points relative to the dense baseline, and SER resumes from the next sparsity step.  ``Baseline'' is the dense top-1 of each pretrained checkpoint as reported by \texttt{timm} \cite{rw2019timm} and the corresponding model/checkpoint sources on ImageNet-1k validation.  The ViT-B/16 row matches the corresponding row of Table~\ref{tab:vitb_current_methods}.}
...
The fitted log-parameter slope steepens monotonically from $-0.73$ pp/decade at $s=0.50$ to $-4.18$ pp/decade at $s=0.70$ ($R \approx -0.4$ across the 17 rows). This systematic ordering --- larger model, smaller drop --- is consistent with the random-matrix view of SER: the MP bulk grows with width while the spike count is roughly architecture-fixed, so a larger model can be pushed deeper into the bulk before the protocol crosses the spike-detection threshold. The full discussion and the per-architecture certificate audit are in \methref{cert-audit}.
...
\end{equation}
where $B_T(R)$ is the data-path budget (see Section~M.\texttt{sec:cert-audit} of the methodology paper for the full per-architecture audit).

In the Gaussian sufficient-condition setting of Corollary~\ref{cor:mp_edge_data_path_budget}, the fitted MP edge $\sigma_+(\ell)$ controls $B_T(R)$ up to a logarithmic factor, motivating the $\sigma_+$-budget allocation used in Hybrid Magnitude--SER. The prune--restore extension (Theorem~\ref{thm:deterministic_prune_restore_certificate}) splits the removed mass into a kept reservoir $Q$ and a finally-removed component $P$, so that restoring $Q$ improves the certificate margin --- this mechanism underlies CASTs ``free restoration (M.\texttt{sec:cast-2e-pipeline}).

The empirical certification audit (M.\texttt{sec:cert-audit} of the methodology paper) verifies three bridges from these results to the multi-architecture numerics: (i) within-arch Pearson correlation between summed $B_T$ and post-projection top-1 ($r=+0.91$ pooled, $+0.6$ to $+0.92$ per-arch); (ii) zero violations of the Lemma~\ref{lem:deterministic_ce_path_bound} upper bound across $282$ audited cells; (iii) architecture-stratified $\sigma_+ \to B$-proxy correlation, strongest in transformers ($r$ up to $+0.83$ for DeiT-Tiny) and weakest for depthwise architectures ($r=-0.29$ for ConvNeXt-Base). The detailed per-architecture comparisons against Table~\ref{tab:multi_arch_hybrid} and Figure~\ref{fig:param_vs_drop} are migrated to the methodology paper to keep the main paper focused on the headline theory and results.


\subsection{Comparison with prior pruning methods (migrated)}
\label{subsec:prior_method_comparison}

Detailed head-to-head comparisons against the leading ViT-B/16 and DeiT pruning baselines --- VTP \cite{zhu2021visual}, PoWER-BERT \cite{goyal2020powerbert}, HVT \cite{pan2021scalable}, CP-ViT \cite{song2022cp}, OViT \cite{kuznedelev2023ovit}, NViT \cite{yang2023nvit}, SViTE \cite{chen2021svite}, Spartan \cite{tai2022spartan}, X-Pruner \cite{yu2023xpruner}, MiniViT \cite{zhang2022minivit}, and AlphaPruning \cite{lu2024alphapruning} --- are migrated to the companion methodology paper to keep the main manuscript focused on theory and headline results; see \methref{seb-protocols} (which continues into the comparison subsections of the migrated content). The headline outcome from those tables is recorded here once: the Hybrid Magnitude--SER protocol matches or improves on every comparable ViT-B/16 baseline at every reported sparsity in $[0.30, 0.70]$ and matches or improves on every DeiT baseline (VTP, PoWER-BERT, HVT, CP-ViT) at every reported sparsity, while requiring no architecture-specific tuning and no validation-set access during pruning.
...
\section{Other Numerical Results (migrated to methodology paper)}
\label{app:fc_num}

This appendix collected numerical illustrations of the simplified theory on Fashion MNIST: MP-based pruning of fully connected DNNs, the outer-layer scaling diagnostics, and the noise-perturbation experiment. The full content of this appendix has been migrated to the companion \emph{methodology and extended-numerics} paper, \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.pdf}{\texttt{cast\_2e/methodology.pdf}} (LaTeX source: \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.tex}{\texttt{cast\_2e/methodology.tex}}); see Section~\textsc{M.}\texttt{sec:fc\_num\_meth} of that paper. The repository root is \url{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning}.
...
\section{Algorithms for RMT-based pruning diagnostics (migrated)}
\label{numerics_updated}

This appendix specified the BEMA edge estimation algorithm, the spectral-fit test, and the cycle-based pruning algorithms used in the classical RMT baseline. The full content of this appendix has been migrated to the companion \emph{methodology and extended-numerics} paper, \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.pdf}{\texttt{cast\_2e/methodology.pdf}} (LaTeX source: \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.tex}{\texttt{cast\_2e/methodology.tex}}); see Section~\textsc{M.}\texttt{sec:bema} of that paper. The repository root is \url{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning}.

\section{Spectral Edge Budgeting and hybrid RMT pruning protocols (migrated)}
\label{app:splus_budget}

This appendix specified the operational SEB and SER protocols, the four-regime strategy, the layer-type extension, the SEB final comparison, the Haar-SV preprocessing sweep, the Hybrid Magnitude--SER protocol, and the checkpoint validation ledgers used in Table~\ref{tab:multi_arch_hybrid}. The full content of this appendix has been migrated to the companion \emph{methodology and extended-numerics} paper, \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.pdf}{\texttt{cast\_2e/methodology.pdf}} (LaTeX source: \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.tex}{\texttt{cast\_2e/methodology.tex}}); see Section~\textsc{M.}\texttt{sec:seb-protocols} of that paper. The repository root is \url{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning}.

\section{CAST: certificate-aware 2:4 + ToMe conversion (migrated)}
\label{app:cast}

This appendix specified the original CAST 2:4 + ToMe pipeline. The generalised CAST-2E $k{:}n$ pipeline, permutation alignment, and inline-FT runner are also documented in the methodology paper (Section~\textsc{M.}\texttt{sec:cast-2e-pipeline}). The full content of this appendix has been migrated to the companion \emph{methodology and extended-numerics} paper, \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.pdf}{\texttt{cast\_2e/methodology.pdf}} (LaTeX source: \href{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning/blob/main/cast_2e/methodology.tex}{\texttt{cast\_2e/methodology.tex}}); see Section~\textsc{M.}\texttt{sec:cast-old-app} of that paper. The repository root is \url{https://github.com/yspennstate/RMT_based_pruning_in_deep_learning}.
