...
\caption{Top-1 accuracy drop after Hybrid Magnitude--SER pruning, plotted against dense parameter count on a log scale, for the 17 model rows retained in Table~\mainref{tab:multi_arch_hybrid} after removing the two partial ViT-Small/16 and Swin-Small rows.  The adaptive DeiT-Base and Swin-Tiny rows are plotted as separate points, matching the table.  Each row is one point; no within-family averaging.  All three panels contain 17 points.  Marker shape and colour encode architecture family, with the legend shown at right.  Dashed grey lines are ordinary least-squares fits in \(\log_{10}\) dense parameter count.  Sign convention: the vertical axis is the unsigned top-1 \emph{drop} (dense\,$-$\,post-FT, positive when the pruned network is worse), and the regression labels in the panels report the same unsigned drop, so a slope whose magnitude grows with sparsity but whose sign appears with the opposite token in the in-panel labels is the unsigned version of the signed fit reported here.  The signed fitted slopes are \(-0.73\), \(-2.40\), and \(-4.18\) pp per decade of parameters at \(s=0.50\), \(s=0.60\), and \(s=0.70\), with Pearson \(R=-0.31\), \(R=-0.43\), and \(R=-0.41\), respectively.  In the signed convention, the slope is negative (larger models have smaller drops) and its magnitude increases with sparsity.}
