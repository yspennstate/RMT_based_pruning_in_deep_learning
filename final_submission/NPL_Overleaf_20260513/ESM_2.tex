...
\newcommand{\methpaper}{\textsc{Online Resource 2}}
\newcommand{\methref}[1]{Section~\ref{#1}}

% -------------------------------------------------
% Title
% -------------------------------------------------
\title{\textbf{Online Resource 2: Methodology and Numerical Supplement}\\[4pt]
...
% to local labels for companion-only references. Online Resource 1 is also
% imported so proof references resolve in this methodology/numerics resource.
\makeatletter
\let\metholdbibcite\bibcite
\let\bibcite\@gobbletwo
\IfFileExists{main.aux}{\externaldocument[main-]{main}}{\externaldocument[main-]{../paper/main}}
\IfFileExists{ESM_1.aux}{\externaldocument[orone-]{ESM_1}}{\externaldocument[orone-]{../paper/ESM_1}}
\let\bibcite\metholdbibcite
\newcommand{\mainref}[1]{\@ifundefined{r@main-#1}{\@ifundefined{r@#1}{\ref*{orone-#1}}{\ref{#1}}}{\ref*{main-#1}}}
\makeatother
...
\medskip
\noindent\textbf{Artifact provenance.}\quad
The release manifest at \path{data/release_manifest.json} in the companion repository records file sizes for the mirrored checkpoints and points to the per-row \texttt{final\_eval.json} files used to populate the tables. SHA-256 checksums for the mirrored 17-model bundle have not been recomputed after the most recent mirroring; readers wishing to verify integrity should regenerate them from the upstream Drive copy linked above. For the ViT-L \(8{:}16\) dense+perm \(85.33\%\) row the archived evidence is the \texttt{final\_eval.json}, intermediate checkpoints, and logs; final post-FT weights for that single row are not mirrored, and that row is reported on the basis of its evaluation JSON and intermediate-checkpoint trail rather than a stored final state dict. The remaining headline rows are backed by stored final checkpoints.
\end{abstract}

\noindent\textbf{Cross-resource conventions.}\quad
We refer to results in the main manuscript as Theorem~X.Y of the main manuscript, Table~Z of the main manuscript, etc.
Norm conventions, theorem numbering, and notation are inherited from \mainpaper{}.

\tableofcontents

% =====================================================
\section{Introduction and how to read this online resource}
\label{sec:intro}
% =====================================================

\subsection{Scope}
\mainpaper{} develops a Marchenko--Pastur (MP)-driven theory of deterministic pruning certificates and reports results: the multi-architecture Hybrid Magnitude--SER table, the CAST 2:4 + ToMe accuracy/throughput row, and the deterministic-certificate audit. Online Resource 1 contains the proof details.
This online resource contains the operational and numerical material:
...
\item the certification-audit numerics (three diagnostics motivated by Section~\mainref{Main_theory_gen} of the main manuscript: top-1 drop vs.\ audit budget, empirical CE exceedances, $\sigma_+ \to B_{\mathrm{proxy}}$ within-cycle correlations);
\item the extended fully-connected MP-pruning experiments;
\item ledgered checkpoint validation tables.
\end{enumerate}

\subsection{When to read which file}
Use \mainpaper{} for the theoretical statements, accuracy and speedup tables, theory-to-numerics mapping table, and high-level reproducibility narrative.

This online resource is intended to support: replicating the reported experiments, understanding the relationship between the relevant code paths in the GitHub repository and the numerical claims in \mainpaper{}, and reading the extended-numerics record.

\subsection{Repository at a glance}
The companion GitHub repository contains:
\begin{itemize}[leftmargin=2em]
\item Approximately 60 Python scripts at the repository root implementing the classical RMT, magnitude, Hybrid Magnitude--SER, four-regime SEB, and drop-threshold protocols (these underlie Section~\mainref{sec:pruning_VITs} of the main manuscript and the multi-architecture validation table);
...
+- cast_2e/                                  # NEW (this online resource's content)
...
\midrule
Table~\mainref{tab:multi_arch_hybrid} of the main manuscript (multi-arch Hybrid Mag--SER) &
\path{hybrid_mag20_then_v8*.py},
\path{run_finetune_magnitude_v3.py},
\path{prune.py} & \S\ref{sec:seb-protocols} \\
Table~\mainref{tab:param_to_flop_followup} of the main manuscript (structured sparsity/FLOP table) &
...
Section~\mainref{sec:pruning_VITs} of the main manuscript, RMT-guided pruning &
\path{magnitude_rmt_sweep.py},
\path{pruning_method_comparison_sweep.py} & \S\ref{sec:section3-detail} \\
Section~\mainref{Main_theory_gen} of the main manuscript, deterministic certificate audit &
...
$k{:}n$ sweep driver (sweeps in this online resource) &
...
\]
For $n=4, k=2$ this is the 6 patterns of CAST (Section~\ref{sec:cast-old-app}); for $n=16, k=8$ it is $\binom{16}{8}=12870$ patterns. When the enumeration becomes prohibitive, \texttt{project\_kn\_sampled.py} provides a sampled variant.

Define the unmasked per-slot source tuple
\begin{equation}
\label{eq:kn-source-tuple}
u_{i,g,k} = \begin{cases}
...
\end{cases}
\end{equation}
and write the materialised retained value and removed value with respect to a candidate pattern $m\in\mathcal{M}_{k:n}$ as
\begin{equation}
\label{eq:kn-slot-values}
v_{i,g,k}(m) = m_k\,u_{i,g,k},
\qquad
r_{i,g,k}(m) = (1-m_k)\,u_{i,g,k}.
\end{equation}
The ``dense'' source corresponds to setting $W^{\mathrm{SER}} \equiv W^{\mathrm{dense}}$ and $M^{\mathrm{SER}} \equiv 0$, in which case $u_{i,g,k} = W^{\mathrm{dense}}_{i,g,k}$.

The within-group calibration covariance and cert-inspired cost are inherited from Eqs.~\ref{eq:cast_covariance}--\ref{eq:cast_cost}:
\begin{equation}
\label{eq:kn-cost}
...
The native $k{:}n$ search treats the $n$ input slots within each group as fixed by the natural channel order. Channel permutations are a known way to improve \(N{:}M\) sparse-network accuracy without changing inference runtime \citep{pool2021channel}; our Cin-permutation variant uses an activation-weighted quartile interleave rather than their search objective. It aligns each layer's input columns by the importance score
\[
...
To bias the per-row cert-cost minimiser toward the Hybrid Magnitude--SER mask without overriding it, we add the following Hamming penalty by generalising the CAST prior:
\begin{equation}
\label{eq:alpha-ser}
c^{\mathrm{prior}}_{i,g}(m) = \alpha_{\mathrm{ser}} \cdot \bar{c}_g \cdot \bigl| m - M^{\mathrm{SER}}_{i,g,:} \bigr|_1,
\qquad
\bar{c}_g = \mathrm{mean}_{i,k}\, u_{i,g,k}^2 \cdot \EE_x\bigl[h_{g,k}(x)^2\bigr].
\end{equation}
...
\item \textbf{UniPTS} \citep{xie2024unipts} unifies post-training sparsity across vision architectures, addressing per-architecture tuning; our cert-aware allocator is shared across families;
...
\end{itemize}
Our work differs from each of these in starting from a deterministic certificate of the data-path budget \(B_T(R)\) in Eq.~(\mainref{eq:data_path_budget}) of the main manuscript, empirically probed by the diagnostic audit of \S\ref{sec:cert-audit}, rather than solely from heuristic mask selection, and in reporting ImageNet-1k checkpoints whose final masks pass either a native \(2{:}4\) Linear audit or an exact im2col+\(2{:}4\) sparse-GEMM audit, while wider \(k{:}n\) rows remain MAC-accounting rows.

Low-bit and binary-parameter networks compress along an axis that is orthogonal to ours: they reduce the precision of stored weights rather than the support of the weight tensors. Recent binary-normalized networks \citep{cabral2025bit} constrain final layer parameters to one bit while using full-precision training states, reporting Food-101 and WikiText-103 experiments; this direction is complementary to our MP-guided pruning and \(k{:}n\) sparse-execution setting, could in principle be composed with it, and is therefore not included as an ImageNet pruning baseline.
...
The exact configurations used for the four FT runs are recorded in \path{cast_2e/code/run_*_ft_inline.py}; we summarise:

\begin{table}[h]
\centering
\caption{Fine-tuning configuration for the 3-epoch distillation runs.}
\label{tab:ft-config}
\scriptsize
...
\begin{table}[h]
\centering
\caption{Pre-FT top-1 accuracy for the selected cell of each (arch, pattern, source) combination. ``Dense'' is the unpruned reference. Numbers are pre-FT only; the post-FT numbers are in Table~\mainref{tab:param_to_flop_followup} of the main manuscript.}
...
\item \emph{ConvNeXtV2 prefers the dense source.} The depthwise structure of ConvNeXtV2 means the SER prior, which is built for dense projections, transfers poorly. The $12{:}16$ dense-source cell has the highest pre-FT value in this sweep, and after FT it produces the $86.35\%$ row of Table~\mainref{tab:param_to_flop_followup} of the main manuscript.
\end{enumerate}
...
\noindent\emph{The BEMA edge-fitting algorithm and the MP-fit metric are the building blocks of every RMT-driven protocol in this online resource and the main manuscript.}



\label{sec:bema_legacy}

\subsection{BEMA-type threshold fit}
\label{finding_lambda_updated}

We outline a BEMA-type fit that estimates the square-case MP bulk scale of \(X=\frac1N W^\top W\) from its eigenvalue ESD and then forms the Tracy--Widom-corrected upper threshold used in \cite{ke2021estimation}. Below is a streamlined square-matrix adaptation.

\begin{algorithm}[H]
\caption{Computing a BEMA-type upper threshold \(T_+\) using MP and Tracy--Widom corrections}
\label{BEMA_algo}
\begin{algorithmic}[1]
...
\State Apply \textsc{BEMAEdge} to estimate the upper threshold \(T_+\).
...
\For{each layer \(W_\ell\) with \(\mathrm{split}_\ell=\mathrm{false}\)}
    ...
    \State Form \(X_\ell=\frac1{N_\ell}W_\ell^\top W_\ell\) and estimate the BEMA upper threshold \(T_+\).
    \State Set \(\sigma_+:=\sqrt{N_\ell T_+}\).
...
\end{subfigure}
\caption{Synthetic BEMA diagnostics: the fitted MP bulk for \(X=\frac1N W^\top W\) and the dependence of the fitted upper threshold on \(\alpha\) and \(\beta\).}
...
\noindent\emph{This section documents the SEB and SER protocols, the four-regime strategy, the layer-type extension, and the final SEB comparison and validation ledgers. Table~\mainref{tab:multi_arch_hybrid} of the main manuscript (multi-architecture Hybrid Magnitude--SER) is the result of this protocol family.}
...
\begin{enumerate}
\item A Hybrid Magnitude--SER sparse checkpoint at sparsity \(s=0.35\), produced by the SEB pipeline of Section~\ref{subsec:hybrid_ser_method}.  The unstructured mask of this checkpoint is denoted \(M^{\mathrm{SER}}\); its construction uses the per-layer Marchenko--Pastur edge signal \(\sigma_+(\ell)\) (Section~\ref{subsec:splus_method}, Eq.~\eqref{eq:splus_weight}) to allocate per-layer pruning weights, so the input to CAST is already a product of the RMT-based pipeline.
...
\]
Define first the unmasked source tuple
\begin{equation}
\label{eq:cast_source_tuple}
u_{i,g,k} \;=\;
\begin{cases}
...
\end{cases}
\end{equation}
and, for a chosen pattern \(m\), materialise the retained slot value and the removed slot value as
\begin{equation}
...
\end{equation}
With the removed slot value $r_{i,g,k}(m)=(1-m_k)\,u_{i,g,k}$ from Eq.~\eqref{eq:cast_slot_values}, gather the per-row removed-weight vector \(r_{i,g}(m)\in\mathbb R^4\) and define the within-group covariance cost
\begin{equation}
\label{eq:cast_cost}
r_{i,g}(m) \;=\; (1-m)\odot u_{i,g,:},
\qquad
c_{i,g}(m) \;=\; r_{i,g}(m)^\top C_g\, r_{i,g}(m)
\;=\;
\frac{1}{|\mathcal C|}\sum_{x\in\mathcal C}\Bigl(\sum_{k:m_k=0} u_{i,g,k}\,h_{g,k}(x)\Bigr)^{\!2}.
\end{equation}
...
\qquad
\bar c_g = \mathrm{mean}_{i,k}\, u_{i,g,k}^2\,\mathrm E_x[h_{g,k}(x)^2],
\end{equation}
...
This section gives the certification-audit numerics in a single self-contained reference. The audit reports three empirical diagnostics motivated by Section~\mainref{Main_theory_gen} of the main manuscript on 18 trained models. These diagnostics track the audit-emitted budget or proxy against measured outcomes; they are not additional proofs of the deterministic certificate.

\subsection{The three bridges}

Recall from Theorem~\mainref{thm:deterministic_mask_certificate} of the main manuscript that for a fixed trained network and a removed-mask $R = W - W \odot M$, the deterministic data-path bound on the cross-entropy change is
\begin{equation}
\label{eq:bt-bridge}
B_T(R) = \frac{2}{|T|} \sum_{s \in T} L_s\, \|R \cdot \psi_1(s)\|_\infty,
\end{equation}
where $L_s$ is the post-layer Lipschitz constant on the data-path of input $s$, $\psi_1(s)$ is the input to the projected layer, and the sum is over the calibration set $T$. Theorem~\mainref{thm:deterministic_mask_certificate} of the main manuscript establishes the inequality $|\Delta\, \mathrm{CE}| \le B_T(R)$ for any mask satisfying the certificate conditions.

\textbf{Bridge 1 (top-1 drop vs.\ audit budget).}
\quad If Theorem~\mainref{thm:deterministic_mask_certificate} of the main manuscript is informative for pruning behavior, then within a single architecture and a single calibration set, larger audit budgets should be associated with larger post-projection top-1 drops. We measure the within-architecture Pearson correlation between $B_T(R_\ell)$ summed over projected layers and the realised pre-FT top-1 drop across cells.
...
The audit reports quantities from Theorem~\mainref{thm:deterministic_mask_certificate} of the main manuscript as diagnostics: across the audited cells there are zero proxy exceedances, but the proxy is not itself a literal verification of the deterministic upper bound. The Bridge~1 correlations show that cells with larger audit budgets have larger post-projection top-1 drops inside an architecture. The Bridge~3 stratification indicates that the MP edge signal $\sigma_+$ is more predictive in transformer families than in ResNet/depthwise families --- consistent with the four-regime SEB analysis in \S\ref{sec:seb-protocols}.

\subsection{Cycle-by-cycle audit data}
The public sweep-result JSONs under \texttt{cast\_2e/sweep\_results/} store projection/top-1 metadata. The CE/proxy audit outcomes used in this online resource are summarized in Table~\ref{tab:cert-audit}; row-level raw audit tables are outside the current public bundle.

% =====================================================
\section{Detailed numerics for Section~\mainref{sec:pruning_VITs} of the main manuscript}
\label{sec:section3-detail}
% =====================================================

\subsection{Per-architecture sparsity ledgers}

The multi-architecture table (Table~\mainref{tab:multi_arch_hybrid} of the main manuscript) reports a single sparsity-target column per architecture family. The full per-architecture sparsity-vs-top-1 trajectories are recorded in two ledger tables:
Table~\mainref{tab:checkpoint_validation_ledger} (the primary ledger, in \S\ref{sec:seb-protocols} of this online resource) and Table~\mainref{tab:additional_downloaded_validation_ledger} (the drop-threshold and restart trajectories).
...
\mainpaper{} carries the Hybrid Magnitude--SER table, the CAST-2E FLOP/speedup table, and a compact theory-to-numerics map. This online resource gives the operational detail behind those rows: per-architecture explanations, ledger tables, cell-sweep JSON schema, certificate-audit plots, and protocol caveats.
...
\noindent The following index tells a reader of \mainpaper{} which section of this online resource to consult for any topic mentioned in the main text.

\begin{table}[h]
\centering
\caption{Quick-reference index from \mainpaper{} topics to this online resource's sections.}
\label{tab:quick-index}
\small
\begin{tabularx}{\textwidth}{lXl}
\toprule
\mainpaper{} mentions \ldots & We document it in this online resource at \ldots & \methpaper{}-\S \\
\midrule
...
Table~\mainref{tab:multi_arch_hybrid} reports the multi-architecture validation ledger in one place.  Unmarked rows use the canonical Hybrid Magnitude--SER protocol (Section~\ref{subsec:hybrid_ser_method}: exact magnitude pruning through \(s=0.20\), SER thereafter); rows marked \(\ddagger\) use the adaptive variant that terminates stage-1 once the post-FT top-1 drops by more than \(0.7\) percentage points relative to the dense baseline.  The displayed table contains 17 model rows after omitting the two partial ViT-Small/16 and Swin-Small ledgers.  The ViT-B/16 row reproduces the Table~\mainref{tab:vitb_current_methods} canonical row for cross-reference.  ResNet101d uses the native-resolution dense baseline measured in this work's evaluation pipeline (256$\times$256, baseline top-1 \(82.26\%\)).

The pattern at the low-sparsity end of Table~\mainref{tab:multi_arch_hybrid} is consistent across architectures: through \(s=0.20\), the exact-magnitude prefix (Section~\ref{subsec:splus_method}) recovers (and at \(s\le 0.10\) often slightly exceeds) the dense baseline after a single fine-tuning epoch on every architecture for which a datapoint is available.  The decisive sparsities for the hybrid protocol lie at \(s\ge 0.45\), where Table~\mainref{tab:vitb_current_methods} establishes that classical magnitude (Section~\ref{subsec:splus_method}) and classical RMT (Section~\ref{subsec:classical_rmt}) cannot match the SER-based methods.  Section~\ref{subsec:checkpoint_validation_ledger} gives the checkpoint-level validation ledger used to populate these tables, and Table~\mainref{tab:additional_downloaded_validation_ledger} records the downloaded threshold and restart ledgers retained for auditability.

\begin{table}[ht]
\centering
\caption{Multi-architecture Hybrid Magnitude--SER results on ImageNet-1k validation.  Entries are post-fine-tuning top-1 accuracy (\%) at sparsity \(s\).  The table contains 17 model rows after omitting the two partial ViT-Small/16 and Swin-Small ledgers.  Unmarked rows use the canonical protocol of Section~\ref{subsec:hybrid_ser_method}: exact global magnitude pruning (Section~\ref{subsec:splus_method}) through \(s=0.20\) followed by SER (Section~\ref{subsec:ser_method}) for \(s\ge0.25\).  Rows marked \(\ddagger\) use the adaptive drop-threshold variant: stage-1 terminates once the post-FT top-1 drops by more than \(0.7\) percentage points relative to the dense baseline, and SER resumes from the next sparsity step.  ``Baseline'' is the dense top-1 measured in this work's ImageNet-1k evaluation pipeline for each pretrained \texttt{timm} checkpoint~\cite{rw2019timm}.  The ViT-B/16 row matches the corresponding row of Table~\mainref{tab:vitb_current_methods}.}
...
The Spectral-Edge Reallocation step of the Hybrid Magnitude--SER pipeline allocates per-layer pruning weights \(w_\ell = \max\bigl(0.1,\, 1+\beta\,d(s)\,z_\ell\bigr)\), where \(z_\ell\) is the layer's standardised \(\sigma_+\) value (Section~\ref{subsec:splus_method}, Eq.~\eqref{eq:splus_weight}).  Layers with \emph{larger} \(\sigma_+\) receive larger \(w_\ell\) and are pruned more aggressively; the design rationale is that, by Corollary~\mainref{cor:mp_edge_data_path_budget}, those are the layers whose data-path budget \(B_T(R_\ell)\) responds most weakly per unit of removed Frobenius mass.  Figure~\mainref{fig:sigma_plus_correlation} reports the within-cycle log-log Pearson correlation \(r\bigl(\log\sigma_+(\ell),\, \log \hat B_{T,\ell}\bigr)\) for every (architecture, sparsity) pair in the audit.
...
\begin{table}[ht]
\centering
\caption{ViT-B/16 reference near 30\% compression on ImageNet-1k validation.  CP-ViT values use the source paper's reported ViT-B/16 rows; the comparison is indicative rather than apples-to-apples because of the checkpoint mismatch (CP-ViT baseline \(77.91\%\) vs.\ our \(85.11\%\)).  Hybrid Magnitude--SER and the three within-work baselines are reproduced from Table~\mainref{tab:vitb_current_methods}.  Rows labeled ``(ours)'' are this work's rows; bold marks notable values, not row ownership.  All entries are post-fine-tuning unless noted; CP-ViT used \(30\) FT epochs vs.\ our cumulative \(\sim 6\) FT epochs through \(s=0.30\).}
...
Hybrid Magnitude--SER (ours)                  & \textbf{84.64} & \(\boldsymbol{-0.47}\) & \textbf{30\%} param \\
\midrule
\multicolumn{4}{c}{\textbf{ViT-B/16 at higher compression} (only this work has direct numbers)} \\
Classical magnitude, \(s=0.50\)               & 81.67 & \(-3.44\) & 50\% param \\
Classical RMT, \(s=0.50\)                     & 82.57 & \(-2.54\) & 50\% param \\
SER, \(s=0.50\)                               & 83.27 & \(-1.84\) & 50\% param \\
Hybrid Magnitude--SER, \(s=0.50\) (ours)      & \textbf{83.37} & \(\boldsymbol{-1.74}\) & \textbf{50\%} param \\
\midrule
...
Hybrid Magnitude--SER, \(s=0.65\) (ours)       & \textbf{79.95} & \(\boldsymbol{-5.16}\) & \textbf{65\%} param \\
Hybrid Magnitude--SER, \(s=0.70\) (ours)       & \textbf{78.01} & \(\boldsymbol{-7.10}\) & \textbf{70\%} param \\
...
\paragraph{Table interpretation.}
The table compares CP-ViT's original ViT-B/16 checkpoint to this work's AugReg checkpoint, so it should be read as context rather than a matched benchmark. The higher-compression rows are included only where this work has direct validation results.
...
\begin{table}[ht]
\centering
\caption{DeiT pruning comparison after fine-tuning.  This is not an apples-to-apples ranking: rows use different dense baselines, compression axes, and training protocols.  ``Top-1'' is the post-pruning ImageNet-1k validation accuracy and ``\(\Delta\)'' is the absolute change from the dense baseline used by that method.  ``Compression'' is FLOPs savings for VTP/PoWER/HVT/CP-ViT and parameter sparsity for Hybrid Magnitude--SER.  VTP, PoWER, HVT, and CP-ViT DeiT values are taken from CP-ViT~\cite{song2022cp}. CP-ViT states that its ImageNet fine-tuning uses \(30\) epochs; the fine-tuning budgets for the VTP/PoWER/HVT fine-tuned values are not reported there. Hybrid fine-tunes for one epoch per sparsity step, totaling \(4\) epochs at \(s=0.20\) and \(7\)--\(8\) epochs at \(s=0.40\). Rows labeled ``(ours)'' are this work's rows; bold marks notable values, not row ownership.}
...
Hybrid Magnitude--SER, \(s=0.20\) (ours) & \textbf{71.86} & \(\boldsymbol{-0.35}\) & \textbf{20\%} param \\
\midrule
...
Hybrid Magnitude--SER, \(s=0.20\) (ours) & \textbf{79.37} & \(\boldsymbol{-0.48}\) & \textbf{20\%} param \\
...
Hybrid Magnitude--SER, \(s=0.20\) (ours) & \textbf{81.46} & \(\boldsymbol{-0.34}\) & \textbf{20\%} param \\
Hybrid Magnitude--SER, \(s=0.35\) (ours) & 81.17 & \(-0.63\) & 35\% param \\
Hybrid Magnitude--SER, \(s=0.40\) (ours) & 80.99 & \(-0.81\) & 40\% param \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^\dagger$Fine-tuned values copied from CP-ViT Table~4; CP-ViT's replication note applies to without-finetuning VTP/PoWER/HVT accuracies.}
\end{tabular}
\end{table}
...
\noindent\emph{This section gives only a short map to the abstract additive perturbation theory in the main manuscript and Online Resource 1. The deterministic mask certificate in Section~\mainref{Main_theory_gen} of the main manuscript is independent of this material.}

\section{Abstract Additive Perturbation Extensions}
\label{sec:abstract-additive-orig}

The main manuscript states the abstract \(W=S+R\) additive framework in Section~\mainref{sec:abstract_additive_extensions}. Online Resource 1 contains the Gaussian/RMT specialization in Section~\mainref{Main_theory} and the detailed proofs in Sections~\mainref{app:proof_perturb}--\mainref{app:proofs_main}. We do not repeat the full assumption and theorem stack here.

For orientation, Assumptions~\mainref{as4}--\mainref{as5} give the local data-path factorization and the abstract concentration condition replacing iid Gaussian concentration; Assumption~\mainref{as6} is used only for optional spectral conclusions. Lemma~\mainref{lem:remove_R_general} and Theorem~\mainref{theorem_reduction_of_loss_general} give the abstract perturbation and loss-reduction statements, Theorem~\mainref{thm:multilayer_telescoping} records the layerwise telescoping extension, and Theorem~\mainref{main_result_reducing_noise_general} gives the stationarity-collapse result. Online Resource 1 records the finite-\(N\) pathwise corollaries and the inverse-\(D\)/BBP-style persistent-spike consequences.
...
\FloatBarrier
\section{Full Literature-Context Table from the Main Manuscript}
\label{sec:full_literature_context_table}

Table~\ref{tab:result_comparison_full} is the full comparison table migrated out of the main article to keep the submitted paper in a concise Letters format. The compact main-paper comparison retains the closest unstructured and structured representations; this table preserves the broader provenance and scope notes for reviewers.
...
\bottomrule
\endlastfoot
\multicolumn{7}{l}{\textit{ViT-B/ViT-L context}} \\
CP-ViT \cite{song2022cp} & ViT-B/16 & \(32.34\%\) FLOP red. & 75.09 & $-2.82$ & 0 & CP-ViT Table~2, baseline \(77.91\%\) \\
CP-ViT \cite{song2022cp} & ViT-B/16 & \(33.62\%\) FLOP red. & 77.36 & $\mathbf{-0.55}$ & 30 & CP-ViT Table~3, same baseline \\
SNOWS \cite{lucas2025snows} & ViT-B/16 & \(2{:}4\) QKV+Out+MLP & 76.57 & $-3.85$ & 0 & one-shot on MiniImageNet-1k; \(1.97\times\) FLOP-count estimate \\
SNOWS \cite{lucas2025snows} & ViT-L/16 & \(2{:}4\) QKV & 81.01 & $-3.16$ & 0 & one-shot on MiniImageNet-1k; no ViT-L endpoint speedup reported \\
MaskLLM (vision/4V) \cite{fang2024maskllm} & ViT-B/16 & \(2{:}4\) learned mask & \(\mathbf{79.46}\) & $\mathbf{+0.31}$ & 20 mask ep. & MaskLLM vision/4V setting (the source paper's title is ``MaskLLM''); weights frozen; no ViT endpoint speedup reported \\
PaPr \cite{mahmud2024papr} & ViT-B/16 AugReg & patch pruning + ToMe & 82.34 & $-2.25$ & 0 & PaPr Table~1; \(53.3\%\) FLOP red.; \(2.15\times\) throughput; token pruning, not weight sparsity \\
PaPr \cite{mahmud2024papr} & ViT-L/16 MAE & patch pruning & \(\mathbf{85.06}\) & $-0.89$ & 0 & PaPr Table~2; \(50.0\%\) FLOP red.; \(\mathbf{2.01\times}\) throughput; token pruning, not weight sparsity \\
ToMe \cite{bolya2023tome} & ViT-L/16 MAE & token merging & \(\mathbf{85.05}\) & $\mathbf{-0.61}$ & MAE FT & ToMe Tables~3/10; \(49.7\%\) FLOP red.; \(\mathbf{1.96\times}\) V100 throughput; token merging, not weight sparsity \\
ToMe \cite{bolya2023tome} & ViT-L/16 MAE & token merging, decreasing & 84.16 & $-1.50$ & MAE FT & ToMe Tables~3/10; \(63.8\%\) FLOP red.; \(\mathbf{2.58\times}\) V100 throughput; token merging, not weight sparsity \\
SparseFormer \cite{gao2024sparseformer} & ViT-B/16 AugReg & latent-token reduction & 83.40 & $-1.20$ & 20+5 ep. & \(64.6\%\) FLOP red.; \(\mathbf{1.85\times}\) throughput; token reduction, not weight sparsity \\
SparseFormer \cite{gao2024sparseformer} & ViT-L/16 AugReg & latent-token reduction & \(\mathbf{85.20}\) & $\mathbf{-0.60}$ & 20+5 ep. & \(69.8\%\) FLOP red.; \(\mathbf{2.60\times}\) throughput; token reduction, not weight sparsity \\
Hybrid Mag--SER (ours) & ViT-B/16 & 30\% unstructured & \(\mathbf{84.64}\) & $\mathbf{-0.47}$ & 1/cycle & Table~\mainref{tab:multi_arch_hybrid}, \(s=0.30\) \\
Hybrid Mag--SER (ours) & ViT-B/16 & 50\% unstructured & \(\mathbf{83.37}\) & $-1.74$ & 1/cycle & Table~\mainref{tab:multi_arch_hybrid}, \(s=0.50\) \\
CAST 2:4+ToMe (ours) & ViT-B/16 & 2:4 + ToMe & \(\mathbf{83.41}\) & $-1.70$ & \textbf{3} & A40 native \(2{:}4\) endpoint \(1.388\times\); separate A100 no-ToMe endpoint \(2.705\times\) \\
CAST 6:12 SER+$\alpha$ (ours) & ViT-B/16 & 6:12 = 50\% & \(\mathbf{83.74}\) & $\mathbf{-1.37}$ & \textbf{3} & accuracy/MAC row; no native 6:12 audit \\
CAST 2:4+ToMe (ours) & ViT-L/16 & 2:4 + ToMe & \(\mathbf{84.37}\) & $-1.47$ & \textbf{3} & A40 native \(2{:}4\) endpoint \(1.394\times\) \\
CAST 8:16 dense+perm (ours) & ViT-L/16 & 8:16 = 50\% & \(\mathbf{85.33}\) & $\mathbf{-0.51}$ & \textbf{3} & accuracy/MAC row; no native 8:16 audit \\
\midrule
\multicolumn{7}{l}{\textit{Other ViT-family deployability context}} \\
GPUSQ-ViT \cite{gpusq_vit2023} & Swin-B & \(2{:}4{+}\)INT8 & 83.40 & $-0.10$ & KD+QAT & \(31\times\) FLOP-count red.; A100 \(1.37\times\) latency and \(2.56\times\) throughput \\
GPUSQ-ViT \cite{gpusq_vit2023} & Swin-B/384 & \(2{:}4{+}\)INT8 & \(\mathbf{84.40}\) & $\mathbf{-0.10}$ & KD+QAT & \(31\times\) FLOP-count red.; A100 \(1.47\times\) latency and \(2.47\times\) throughput \\
\midrule
\multicolumn{7}{l}{\textit{CNN and ConvNeXt context}} \\
Zhu--Gupta \cite{zhu2017prune} & InceptionV3 & 50\% unstructured & \(\mathbf{78.0}\) & $\mathbf{-0.1}$ & \(n/r\) & model-size result; no endpoint speed row \\
Zhu--Gupta \cite{zhu2017prune} & MobileNetV1 & 50\% unstructured & 69.5 & $-1.1$ & \(n/r\) & also reports 75\% sparsity at 67.7; no endpoint speed row \\
SFP \cite{he2018softfilter} & ResNet-50 & \(41.8\%\) FLOPs & 74.61 & $-1.54$ & from scratch; \(n/r\) ep. & filter/channel-pruned CNN \\
HRank \cite{lin2020hrank} & ResNet-50 & \(43.8\%\) FLOPs & 74.98 & $-1.17$ & 30 ep./block & feature-map-rank filter pruning \\
FPGM \cite{he2019fpgm} & ResNet-50 & \(53.5\%\) FLOPs & 74.83 & $-1.32$ & FT; \(n/r\) ep. & geometric-median filter pruning \\
ResRep \cite{ding2021resrep} & ResNet-50 & \(54.5\%\) FLOPs & \(\mathbf{76.15}\) & $\mathbf{0.00}$ & 180 train & structural re-parameterization; narrowed dense CNN \\
DepGraph \cite{fang2023depgraph} & ResNet-50 & \(51.8\%\) MACs & \(\mathbf{75.83}\) & $-0.32$ & FT; \(n/r\) ep. & automatic structural pruning \\
Mishra et al. \cite{mishra2021accelerating} & ResNet-50 & 2:4 FP16 & \(\mathbf{76.20}\) & $\mathbf{+0.10}$ & repeated train & original \(2{:}4\) paper; up to \(\mathbf{2\times}\) sparse Tensor Core math \\
Mishra et al. \cite{mishra2021accelerating} & ResNet-50 SWSL & 2:4 FP16 & \(\mathbf{80.90}\) & $\mathbf{-0.20}$ & repeated train & high-accuracy original-paper reference; up to \(\mathbf{2\times}\) math \\
Mishra et al. \cite{mishra2021accelerating} & ResNet-101 & 2:4 FP16 & \(\mathbf{78.00}\) & $\mathbf{+0.30}$ & repeated train & original \(2{:}4\) paper; up to \(\mathbf{2\times}\) math \\
Pool--Yu perm. \cite{pool2021channel} & ResNet-50 & \(2{:}4{+}\)perm & \(\mathbf{76.29}\) & $\mathbf{+0.13}$ & repeated train/FT & no inference-time overhead; V100 permutation search time \(02{:}13\) is small relative to a full train+FT cycle \\
SR-STE \cite{zhou2021nm} & ResNet-50 & 2:4 & 77.00 & $-0.30$ & 120 train & \(N{:}M\) sparse training from scratch; 2.15G test FLOPs \\
LBC \cite{zhang2022lbc} & ResNet-50 & 2:4 & \(\mathbf{77.20}\) & $\mathbf{+0.10}$ & 120 train & learns \(N{:}M\) combinations; \(0.72\times\) training FLOPs \\
AC/DC \cite{peste2021acdc} & ResNet-50 & \(2{:}4\) & \(\mathbf{76.64}\) & $\mathbf{-0.20}$ & 100 train & semi-structured AC/DC sparse training; no endpoint speed reported \\
AC/DC \cite{peste2021acdc} & ResNet-50 & 50\% unstructured & \(\mathbf{77.05}\) & $\mathbf{+0.21}$ & 100 train & global pruning result; no 50\% speed row \\
MIS \cite{yu2021mis} & ResNet-50 & 70\% fine-grained & \(\mathbf{76.56}\) & $\mathbf{+0.43}$ & distill/FT & supervised MIS; 70\%-FINE/AGP baseline also \(76.50\); separate GPU speed ranges are not tied to this supervised row \\
Random pruning \cite{liu2022randompruning} & WRN-50 & 60/70\% unstructured & 77.55/76.81 & $-0.56/-1.30$ & sparse train & SNIP-ratio layer allocation; ResNet-family, not vanilla ResNet-50 \\
UniPTS \cite{xie2024unipts} & ResNet-50 & 50/60/70\% unstructured & 75.76/75.37/74.73 & $-0.36/-0.75/-1.39$ & 16k iters & post-training sparsity with 10,240 ImageNet calibration images; no speed row \\
STR \cite{kusupati2020str} & ResNet-50 & \(79.55/81.27\%\) unstructured & 76.19/76.12 & $-0.82/-0.89$ & train & nearby 80\%+ sparsity evidence; 766/705M reported FLOPs, not a 50--70\% row \\
HALP \cite{shen2022halp} & ResNet-50 & \(51.2\%\) FLOP red. & \(\mathbf{76.50}\) & $\mathbf{+0.30}$ & 90 FT & \(1.60\times\) Titan V; TensorRT FP32 \(1.80\times\) \\
HALP \cite{shen2022halp} & ResNet-101 & \(53.8\%\) FLOP red. & \(\mathbf{77.80}\) & $\mathbf{+0.40}$ & 90 FT & \(1.60\times\) Titan V structural-pruning endpoint \\
NVIDIA TRT INT8 \cite{nvidia2023int8sparsity} & ResNet-34 & \(2{:}4{+}\)INT8 & 73.16 & $-0.07$ & \(n/r\) & \(1.36\times\) A40 TensorRT INT8; batch sweep up to \(1.40\times\) \\
MDP \cite{sun2025mdp} & ResNet-50 & \(\sim73\%\) FLOPs & 74.80 & $-1.40$ & 90 FT & \(3.06\times\) Titan V measured speed; stronger reduction, larger drop \\
Group Fisher \cite{liu2021groupfisher} & ResNet-50 & \(50.0\%\) FLOPs & \(\mathbf{76.42}\) & $-0.37$ & FT as dense & \(1.79\times\) 2080 Ti measured inference speed \\
OVW \cite{tan2022ovw} & ResNet-50 & 50\% column-vector sparsity & 75.76 & $-0.36$ & 20 FT & \(1.86\times\) V100 sparse-conv speedup \\
Isomorphic \cite{fang2024isomorphic} & ResNet-50 & \(50.1\%\) MAC red. & \(\mathbf{75.91}\) & $\mathbf{-0.22}$ & 100 FT & structural pruning; dense-kernel model \\
Isomorphic \cite{fang2024isomorphic} & ResNet-101 & \(51.0\%\) MAC red. & \(\mathbf{77.43}\) & $\mathbf{+0.05}$ & 100 FT & structural pruning; dense-kernel model \\
Isomorphic \cite{fang2024isomorphic} & ResNet-152 & \(64.9\%\) MAC red. & \(\mathbf{77.84}\) & $\mathbf{-0.47}$ & 100 FT & structural pruning; dense-kernel model \\
CAP \cite{kuznedelev2023cap} & ResNet-50D & 50/60\% unstructured & \(\mathbf{79.8/79.7}\) & $\mathbf{-0.7/-0.8}$ & 300-epoch gradual & dense \(80.5\%\); no endpoint speed row \\
Isomorphic \cite{fang2024isomorphic} & ConvNeXt-B\(\to\)S & \(44.8\%\) MAC red. & \(\mathbf{83.17}\) & $\mathbf{-0.66}$ & 300 FT & pruned ConvNeXt-style dense model \\
Isomorphic \cite{fang2024isomorphic} & ConvNeXt-B\(\to\)T & \(72.7\%\) MAC red. & 82.19 & $-1.64$ & 300 FT & pruned ConvNeXt-style dense model \\
CAP \cite{kuznedelev2023cap} & ConvNeXt-L CLIP & 50/60/70\% unstructured & \(\mathbf{87.5/87.1/86.8}\) & $\mathbf{-0.3/-0.7/-1.0}$ & 0 & one-shot pruning; dense \(87.8\%\); no endpoint speed row \\
OWL+Wanda \cite{yin2024owl} & ConvNeXt-B & 50/60/70\% unstructured & 82.76/80.53/68.28 & -- & 0 & appendix one-shot vision baseline; 70\% collapses; no vision speed row \\
SST slices \cite{taka2025sst} & ConvNeXt-S & mixed dense/\(2{:}4\) & 82.62 & $-0.47$ & \(n/r\) & \(1.93\times\) FPGA in-fabric estimate; BF16, depthwise layers dense \\
Hybrid Mag--SER (ours) & ResNet50 & 50/60/70\% unstructured & \(\mathbf{75.76}\)/74.83/74.15 & $-0.37/-1.30/-1.98$ & 1/cycle & Table~\mainref{tab:multi_arch_hybrid}, \(s=0.50/0.60/0.70\) \\
Hybrid Mag--SER (ours) & ConvNeXt-B & 50/60/70\% unstructured & \(\mathbf{84.80}\)/83.70/81.21 & $-1.04/-2.14/-4.63$ & 1/cycle & Table~\mainref{tab:multi_arch_hybrid}, \(s=0.50/0.60/0.70\) \\
Hybrid Mag--SER (ours) & ConvNeXtV2-B & 50/60/70\% unstructured & \(\mathbf{85.33}\)/84.61/81.40 & $-1.39/-2.11/-5.32$ & 1/cycle & Table~\mainref{tab:multi_arch_hybrid}, \(s=0.50/0.60/0.70\) \\
CAST-conv+perm (ours) & ResNet50 & 2:4 im2col & \(\mathbf{75.67}\) & $\mathbf{-0.46}$ & \textbf{3} & \(\mathbf{1.700\times}\) A40 im2col audit; not cuDNN Conv2d \\
CAST-conv+perm (ours) & ResNet50d & 2:4 im2col & 78.00 & $-2.55$ & \textbf{3} & \(1.496\times\) A40 im2col audit; not cuDNN Conv2d \\
CAST-conv+perm (ours) & ResNet101d & 2:4 im2col & \(\mathbf{80.59}\) & $-1.67$ & \textbf{3} & \(\mathbf{1.568\times}\) A40 im2col audit; not cuDNN Conv2d \\
CAST-conv+perm (ours) & ResNet152d & 2:4 im2col & \(\mathbf{81.33}\) & $-1.53$ & \textbf{3} & \(\mathbf{1.617\times}\) A40 im2col audit; stronger timm baseline \\
CAST 2:4 (ours) & ConvNeXtV2-B & 50\% MAC-accounting & \(\mathbf{85.47}\) & $-1.25$ & \textbf{3} & \(1.295\times\) A40 native Linear endpoint; dense convs unchanged \\
CAST 8:16 (ours) & ConvNeXtV2-B & 50\% MAC-accounting & \(\mathbf{85.85}\) & $-0.87$ & \textbf{3} & accuracy/MAC row; no native 8:16 audit \\
CAST 12:16 (ours) & ConvNeXtV2-B & \(\sim\)25\% MAC-accounting & \(\mathbf{86.35}\) & $\mathbf{-0.37}$ & \textbf{3} & accuracy/MAC row; no native 12:16 audit \\
\end{longtable}
\endgroup

\FloatBarrier
\section{Migrated Main-Manuscript Numerical Appendices}
\label{sec:migrated_main_numerical_appendices}
...
Table~\mainref{tab:reproduce_recipe} records the compact recipe summary for the main structured-projection rows.

\begin{table}[!htbp]
\centering
\caption{Hyperparameters for representative CAST/CAST-2E rows of Table~\mainref{tab:param_to_flop_followup}. The table lists the source checkpoint for each run and the post-projection distillation schedule. The CAST projection (Algorithm~\mainref{alg:cast_kn}) is gradient-free.}
...
\midrule
\textbf{Post-FT top-1} & \multicolumn{4}{c}{Reported in Table~\mainref{tab:param_to_flop_followup}} \\
\bottomrule
\end{tabularx}
\end{table}
...
Training budgets already shown in Table~\mainref{tab:result_comparison} are not repeated here. Zero-fine-tuning author-run one-shot baselines and external rows without a reported training/fine-tuning budget are omitted from the main comparison table; the omitted values are SparseGPT-style \(50\%\) unstructured ViT-B/16 no-FT at \(\sim81.1\%\) (\(\sim-4.0\) pp) and Wanda-style at \(\sim81.3\%\) (\(\sim-3.8\) pp). DeiT-only comparison rows are not used in the main comparison because the headline rows here are ViT-B/16, ViT-L/16, ResNet, and ConvNeXt-family checkpoints. SNOWS evaluates on MiniImageNet-1k subsets rather than the full ImageNet-1k validation set. CP-ViT's ViT-B/16 entries in Table~\mainref{tab:result_comparison} use the CP-ViT-reported \(77.91\%\) dense baseline. For this work's rows, the prunable-layer list is fixed across the multi-architecture sweep; masks are constructed without ImageNet validation access; validation is used only to report completed checkpoints and in documented global-protocol sweeps. Results are reported against the corresponding dense checkpoint measured in this work's evaluation pipeline.

Table~\mainref{tab:deployability_audit_compact} reports the selected best-observed A40 batch-sweep endpoints; batch \(256\) was also swept but not selected. The different A100 no-ToMe ViT-B endpoint remains separate: \(463.6\to1254.1\) images/s, \(2.705\times\), and should not be pooled with the ToMe-held-fixed A40 audits. ToMe-only AugReg references from \cite{bolya2023tome} are \(83.94\%\) at \(r=8\) and \(82.86\%\) at \(r=12\).

Extending the ResNet endpoint caveat in Table~\mainref{tab:deployability_audit_compact}, the im2col+\(2{:}4\) rows do not establish TensorRT or cuDNN Conv2d acceleration. The \(1\times1\)-only controls remain below native cuDNN Conv2d throughput; see \methref{subsec:6to24}. A ResNet152d \(8{:}16\) dense+perm row is outside the reported fixed-calibration sweep: the wider-pattern certificate projection exceeded a single 80\,GB A100 at the default calibration batch.
...
The public artifact bundle is indexed by \texttt{data/release\_manifest.json}; Online Resource 2 gives protocol-level provenance and ledger details. CAST row estimates are primarily single-seed; the available replicate summaries provide the repeatability check for the reported CAST setting.
...
The slope-steepening and bulk-reservoir interpretation of Figure~\mainref{fig:param_vs_drop} are discussed in \methref{sec:s34-arch-validation} and \methref{sec:s35-cert-numerics}.
...
The deterministic mask certificate (Theorem~\mainref{thm:deterministic_mask_certificate}) certifies that pruning a removed component $R$ from a layer $A=S+R$ decreases the elastic-net objective whenever the data-path budget satisfies
\begin{equation}
\label{eq:certificate_inequality_summary}
B_T(R)\;<\;\lambda_2\|R\|_F^2+\lambda_1\|R\|_1,
\end{equation}
...
Operational SEB/SER protocols, four-regime strategy, layer-type extension, SEB final comparison, Haar-SV preprocessing sweep, Hybrid Magnitude--SER protocol, and checkpoint validation ledgers used in Table~\mainref{tab:multi_arch_hybrid} are specified in Online Resource 2.
...
\State Compute layerwise weights $w_\ell(s) = \max(0.1, 1 + \beta\,d(s)\,z_\ell)$ where
       ...
       $d(s) = (1 - s/s_{\rm d})_+^p$. \Comment{Section~\mainref{sec:pruning_VITs}}
...
\Require
    ...
    calibration set $\mathcal{C}$ (row-specific size in Table~\mainref{tab:reproduce_recipe});
...
\paragraph{Justification of Algorithm~\mainref{alg:cast_kn}.}
...
Lemma~\mainref{lem:deterministic_ce_path_bound} bounds \(|\Delta\,\mathrm{CE}|\)
...
Theorem~\mainref{thm:deterministic_prune_restore_certificate}: slots that the unstructured
...
bound under the conditions of Corollary~\mainref{cor:restoration_improves_certificate}. The $\alpha_{\rm ser}$ prior pulls the chosen pattern towards the
