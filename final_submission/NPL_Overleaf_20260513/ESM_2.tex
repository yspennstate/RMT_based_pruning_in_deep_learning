...
The materialised slot value retains CAST's three-case rule (Eq.~\ref{eq:cast_slot_values}):
\begin{equation}
...
\end{itemize}
Our work differs from each of these in starting from a deterministic certificate of the data-path budget \(B_T(R)\) in Eq.~(\mainref{eq:data_path_budget}) of the main manuscript, empirically probed by the diagnostic audit of \S\ref{sec:cert-audit}, rather than from a heuristic mask choice, and in reporting ImageNet-1k checkpoints whose final masks pass either a native \(2{:}4\) Linear audit or an exact im2col+\(2{:}4\) sparse-GEMM audit, while wider \(k{:}n\) rows remain MAC-accounting rows.
