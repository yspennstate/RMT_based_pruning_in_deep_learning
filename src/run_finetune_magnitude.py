...
OUT_DIR = Path("/workspace/finetune_results_magnitude")
...
SEED             = 42
SKIP_PRE_FT_EVAL = os.environ.get("RMT_SKIP_PRE_FT_EVAL", "0").lower() in {"1", "true", "yes", "y", "on"}
...
def main():
    ...
    for cycle_idx in range(start_cycle, len(CYCLES)):
        target_s, epochs, base_lr = CYCLES[cycle_idx]
        t0 = time.time()
        log("-" * 70)
        ...
        if SKIP_PRE_FT_EVAL:
            pre_ft_top1 = None
            log(f"  eval pre-FT s={target_s:.2f}: skipped")
        else:
        ...
        save_results(results)
        if pre_ft_top1 is None:
            log(f"  cycle {cycle_idx+1} summary: pre-FT=skipped  post-FT={post_ft_top1:.2f}%  "
