...
OUT_DIR = Path("/workspace/finetune_results")
...
CYCLES = [
    (0.05, 1, 3.00e-5, "alpha_budget_b0.50_sd0.30",                   True),
    (0.10, 1, 3.00e-5, "alpha_budget_b0.50_sd0.30",                   False),
    (0.15, 1, 3.00e-5, "alpha_budget_b0.50_sd0.30",                   False),
    (0.20, 1, 3.00e-5, "alpha_budget_b0.50_sd0.30",                   False),
    (0.25, 1, 3.00e-5, "splus_budget_b1.00_sd0.50",                   False),
    (0.30, 2, 3.00e-5, "splus_budget_b1.00_sd0.50",                   False),
    (0.35, 2, 3.00e-5, "splus_budget_b1.00_sd0.50",                   False),
    (0.40, 2, 3.00e-5, "splus_budget_b1.00_sd0.50",                   False),
    (0.45, 3, 2.50e-5, "splus_budget_b1.25_sd0.70_p1.0",              False),
    (0.50, 3, 2.50e-5, "splus_budget_b1.25_sd0.70_p1.0",              False),
    (0.55, 3, 2.00e-5, "splus_budget_b1.25_sd0.70_p1.0",              False),
    (0.60, 3, 1.75e-5, "splus_budget_b1.50_sd0.85_p1.0_ba1.00_bm2.00", False),
    (0.65, 3, 1.50e-5, "splus_budget_b1.50_sd0.85_p1.0_ba1.00_bm2.00", False),
    (0.70, 3, 1.25e-5, "splus_budget_b1.50_sd0.85_p1.0_ba1.00_bm2.00", False),
]
...
WEIGHT_DECAY     = 1e-6
WARMUP_STEPS     = 500
MIN_LR_FRACTION  = 0.01  # final LR = base_lr * this
SEED             = 42
...
def prune_to_target(model, target_sparsity, method_str, apply_sv, signals):
    """Apply regime pruning to reach target_sparsity. Previously zeroed weights
    stay zeroed because magnitude selects smallest-magnitude first.
    """
    signal_key, beta_max, s_decay, decay_power, beta_attn, beta_mlp = parse_method_name(method_str)

    if apply_sv:
        log(f"  SV preprocessing (Haar z=0.5 p=3)...")
        apply_sv_preprocessing(model, signals)

    layer_weights = layer_weights_from_signal(
        signals, signal_key, beta_max, s_decay,
        target_sparsity, decay_power, beta_attn, beta_mlp
    )
    model.cpu()
    achieved = apply_modulated_magnitude(model, target_sparsity, layer_weights)
    model.to(DEVICE)
    return achieved
...
def main():
    ...
    log("computing per-layer RMT signals (σ+, Hill-α, etc.)...")
    signals = compute_layer_signals(model)
