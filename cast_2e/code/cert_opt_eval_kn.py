...
Also reports the natural Cin-eligibility per (n) since 4:8 needs Cin % 8 == 0.

Usage:
  python cert_opt_eval_kn.py \\
    ...
    --imagenet-train-for-calib /workspace/imagenet/train \\
...
def load_ser_with_coverage(model: nn.Module, ckpt_path: str, *, raise_on_low: bool = True):
    ...
    if raise_on_low and coverage < 0.95:
...
def main():
    ...
    print(f"[cert_opt_eval_kn] device={device}, model={args.timm_name}")
    import timm
    ...
    # Two source state-dicts: dense pretrained (= teacher), and SER s=0.35
    print("[cert_opt_eval_kn] capturing dense state...")
    dense_state = {n_: p.detach().clone() for n_, p in teacher.state_dict().items()}

    print(f"[cert_opt_eval_kn] loading SER ckpt: {args.ser_checkpoint}")
    ...
    print("[cert_opt_eval_kn] eval source baselines...")
    t0 = time.time()
    ...
    # Cells: (label, k, n, source, perm, alpha_ser)
    # `source`: "dense" or "ser"
    cells = [
        # === Direct dense vs SER comparison at 2:4 baseline ===
        ("D24_dense_perm_no_ser",    2, 4, "dense", True,  0.0),  # dense source, perm only
        ("D24_dense_no_perm_no_ser", 2, 4, "dense", False, 0.0),  # dense source, nothing
    ...
    for label, k, n, source, perm, alpha_ser in cells:
        ...
        cert_stats = cert_aware_kn_for_conv(
            ...
            n=n, k=k,
            dense_state_dict=dense_state,        # always for free_restoration
            n_calib_imgs=64,
