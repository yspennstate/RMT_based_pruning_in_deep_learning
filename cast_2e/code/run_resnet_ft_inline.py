"""
run_resnet_ft_inline.py — runs cert-aware k:n projection AND 3-ep distillation FT
for ResNet (Conv2d pipeline). Same pattern as run_vitb_ft_inline.py but uses
cert_aware_kn_for_conv to handle Conv2d layers. Inline (no save/load roundtrip)
to avoid the perm-hook serialization bug.

Usage:
    python run_resnet_ft_inline.py \\
        ...
        --output-dir /workspace/run_outputs/resnet_ft/D48_dense_perm \\
        --k 4 --n 8 --source dense --alpha-ser 0.0 \\
        --epochs 3 --batch 256 --lr 1e-3
"""
...
from project_kn_sparsity import cert_aware_kn_for_conv
...
def main():
    ...
    ap.add_argument("--only-1x1", action="store_true", default=False,
                    help="Only project 1x1 convs (faster, less FLOP reduction)")
    ...
    print(f"[ft_resnet_inline] device={device}, model={args.timm_name}")
    print(f"  cert: k:n={args.k}:{args.n}, source={args.source}, alpha_ser={args.alpha_ser}, "
          f"perm={args.permute_align}, free_restore={args.free_restoration}, only_1x1={args.only_1x1}")
    print(f"  ft: epochs={args.epochs}, batch={args.batch}, lr={args.lr}, momentum={args.momentum}")
    import timm
    ...
    crop_pct = cfg.get("crop_pct", 0.875); interp = cfg.get("interpolation", "bilinear")

    # 2. Loaders — timm canonical with light aug for ResNet (no RandAug, just random-resize-crop + flip)
    ...
    train_tx = timm.data.create_transform(
        ...
        auto_augment=None,  # ResNets typically don't use RandAug
        re_prob=0.0, hflip=0.5, color_jitter=0.0,
    )
    ...
    # 4. Run cert-aware k:n projection in-place on Conv2d layers (registers _cin_perm hooks)
    print(f"\n=== Running cert-aware {args.k}:{args.n} projection (Conv2d pipeline) ===")
    t0 = time.time()
    cert_stats = cert_aware_kn_for_conv(
        ...
        alpha_ser_prior=args.alpha_ser, only_1x1=args.only_1x1,
    ...
    # 7. Optimizer (SGD+momentum is canonical for ResNets)
    decay, no_decay = [], []
    for n_, p in student.named_parameters():
        if not p.requires_grad: continue
        if p.ndim <= 1 or "bn" in n_.lower():
            no_decay.append(p)
        else:
            decay.append(p)
    ...
    # 8. FT loop with KD distillation
    ...
    final = {
        ...
        "cert_config": {"k": args.k, "n": args.n, "source": args.source,
                         ...
                         "free_restoration": args.free_restoration, "only_1x1": args.only_1x1,
                         "pipeline": "conv"},
        "ft_config": {"epochs": args.epochs, "lr": args.lr, "batch": args.batch,
                      "weight_decay": args.weight_decay, "momentum": args.momentum,
                      "distill_temp": args.distill_temp, "distill_alpha": args.distill_alpha,
                      "label_smoothing": args.label_smoothing, "warmup_steps": args.warmup_steps},
