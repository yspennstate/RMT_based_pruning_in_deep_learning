...
~50 cells per arch covering:
  ...
  - α_ser sweep: 0.0, 0.1, 0.3, 0.5, 0.7, 1.0
  - perm: on (mostly), off (some)
  - calib: 64, 256 (variance probe)
  - 5-seed replicate of winning cell

Same CLI as cert_opt_eval_kn.py.
"""
...
def load_ser(model, ckpt_path, raise_on_low=True):
    ...
    if raise_on_low and cov < 0.95:
        raise RuntimeError(f"coverage {cov:.4f} < 0.95")
...
def build_cells():
    """~50 cells per arch."""
    cells = []
    # Cell tuple: (label, k, n, source, perm, alpha_ser, calib, seed_offset)
    # === Pass A: 2:4 baseline + dense vs SER + α_ser sweep ===
    ...
    # === Pass E: calib variance probes (2:4 ser_0.5) ===
    for calib in [256, 512]:
        cells.append((f"S24_perm_a050_c{calib}", 2, 4, "ser", True, 0.5, calib, 0))
    # === Pass F: replicates for variance bound (2:4 perm a050 dense) ===
    for r in range(5):
        cells.append((f"S24_perm_a050_rep{r}", 2, 4, "ser", True, 0.5, 64, r))
    # === Pass G: 2:4 dense replicates ===
    for r in range(3):
        cells.append((f"D24_perm_a000_rep{r}", 2, 4, "dense", True, 0.0, 64, r))
    return cells
...
def main():
    ...
    print(f"[cert_opt_eval_kn_extended] device={device}, model={args.timm_name}")
    import timm
    ...
    cells = build_cells()
    print(f"[cert_opt_eval_kn_extended] {len(cells)} cells planned")
    ...
    for label, k, n, source, perm, alpha_ser, calib, seed in cells:
        ...
        try:
            ...
            print(f"  pre-FT top1 = {pre_ft_top1:.4f}  bad={bad}  layers={n_layers}  "
                  f"projected_in={time.time()-t_start-(time.time()-t_eval):.0f}s  eval_in={time.time()-t_eval:.0f}s")
