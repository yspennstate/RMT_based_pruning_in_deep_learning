...
This subdirectory contains the **companion methodology paper**, the CAST-2E
code, result JSONs, and speedup benchmarks that support the FLOP table in the
main manuscript.
...
## Reproducing a reported number end-to-end

```bash
# ViT-B 6:12 SER+α=0.5 (83.74% reported top-1)
...
# ResNet50.tv 8:16 dense+perm (75.67% reported top-1)
...
[`AUDIT.md`](AUDIT.md) and planned for a HuggingFace Hub release after the
paper is finalised. Inference and reproducibility verification can use the
projection script + a fresh 3-epoch FT to reproduce each post-FT number
end-to-end.
