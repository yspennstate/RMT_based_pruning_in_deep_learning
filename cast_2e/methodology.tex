...
\texttt{haar\_optuna*.py} & \S\ref{sec:seb-protocols}.\mainref{subsec:alpha_signal_meth} \\
...
CAST (\textbf{C}ertificate-\textbf{A}ware \textbf{S}parse-\textbf{T}oken conversion) is the procedure used to produce the second row of Table~\mainref{tab:param_to_flop_followup}.  This appendix records the algorithmic details, the connections to the deterministic certificate framework of Section~\mainref{Main_theory_gen}, and the role played by the RMT/SER pipeline that supplies CAST's input checkpoint.
...
\begin{itemize}
\item The cost in Eq.~\eqref{eq:cast_cost} is the \(\ell^2\) form of Lemma~\mainref{lem:deterministic_ce_path_bound}'s removed-contribution quantity, not its literal \(\ell^\infty\) form.  Exact \(\ell^\infty\) rescoring of the top-K riskiest groups is a small follow-up.
\item The post-layer Lipschitz factor \(L_s\) of Lemma~\mainref{lem:deterministic_ce_path_bound} is held constant; computing it per-sample via a single backward pass would tighten the cost.
...
% =====================================================
\section{Migrated discussion: \mainpaper{}-\S3 interpretation}
\label{sec:s33-interpretation}
% =====================================================

\noindent\emph{The following paragraphs were the original \mainpaper{}-\S3.3
interpretation block. The main manuscript now carries only a one-paragraph
summary; the full text appears here for archival completeness.}
...
% =====================================================
\section{Migrated discussion: broader architecture validation}
\label{sec:s34-arch-validation}
% =====================================================

\noindent\emph{The following text is the original \mainpaper{}-\S3.4. The
main manuscript retains only Table~2 (the multi-architecture validation
ledger) and the parameter-vs-drop figure. The verbose architectural
discussion --- which rows are stress tests, the protocol-equivalence
caveat, and the per-architecture interpretation of the parameter-vs-drop
slope --- is migrated here.}
...
This empirical pattern is suggestive of, though not confirmatory for, the RMT picture.  The predicted directional sign --- bigger model, smaller drop --- is observed in every panel; the magnitude of the slope grows roughly in lockstep with sparsity.  Most checkpoints remain close to the dense baseline at moderate sparsity, and the large departures appear once the protocol begins to remove components close to or beyond the MP edge --- the regime where classical RMT prediction starts to bite.  We do not interpret Figure~\mainref{fig:param_vs_drop} as direct confirmation of the bulk-vs-spike decomposition; that would require per-layer measurements of the certificate quantities, which we report in Section~\mainref{subsec:theory_numerics_alignment} below.  We do interpret it as evidence consistent with the central premise of the paper: as networks grow, the bulk-vs-spike decomposition that motivates SER appears to become a better description of where the prunable capacity lives, so the same protocol becomes more accuracy-preserving at scale.



% =====================================================
\section{Migrated discussion: connecting deterministic certificates to multi-architecture numerics}
\label{sec:s35-cert-numerics}
% =====================================================

\noindent\emph{The following text is the original \mainpaper{}-\S3.5
``Connecting the deterministic certificates to the multi-architecture
numerics'' subsection. It runs through the certificate-side prediction,
constructs the layerwise data-path budget proxy, and walks through the
parameter-vs-drop curve at every reported sparsity. The main manuscript
retains a 5-line summary; the full discussion --- including the per-cycle
$\sigma_+ \to B_{\mathrm{proxy}}$ regression and the $L_s$ Lipschitz
bound construction --- is migrated here.}
...
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize \textsuperscript{\dag}ViT-L/16 reduction is estimated from the analytical 22.81\% ToMe + 50\% 2:4-Linear share. \textsuperscript{\ddag}\textbf{CAST-conv+perm} = CAST-conv with the importance-balanced Cin permutation alignment of Appendix~\mainref{app:perm} (``flatten the layer''), motivated by the ResNet pre-FT collapse paragraph below.  ResNet152d row was added in this revision; CAST-conv+perm achieves \(81.33\%\) post-FT on \texttt{resnet152d.ra2\_in1k} (dense baseline \(82.86\%\)), a \(-1.53\) pp drop at \(\sim\!50\%\) sparse-execution MAC reduction.  \textsuperscript{\S}New rows added in this revision (May 9): (i) ViT-B/16 \textbf{6:12 SER+\(\alpha\)=0.5 inline} — same 50\% N:M structured sparsity as the canonical 2:4 row but at \(n=12\) (more pattern flexibility per group), no ToMe, dense-teacher distillation; this lands at \(83.74\%\) post-FT, \(\mathbf{+0.33}\) pp above the canonical \(2{:}4{+}\)ToMe row at the same parameter sparsity, demonstrating the n-flexibility benefit of wider N:M patterns. (ii) ConvNeXtV2-Base \textbf{12:16 dense+perm} — 25\% sparsity (vs 50\% for 2:4); reaches \(86.35\%\) post-FT, only \(-0.37\) pp from dense \(86.72\%\), the ``minimal accuracy loss'' density lane.}
\end{table}
...
Every algorithmic step in the CAST and CAST-conv+perm pipelines maps to a specific certificate-improving step in the deterministic mask-certificate framework of Section~\mainref{Main_theory_gen}.  Table~\mainref{tab:cast_theory_map} pairs each step with the theorem or lemma that motivates it; the table is read as a one-to-one correspondence between an empirical knob in the implementation and a certificate-margin claim in the theory.

\begin{table}[ht]
\centering
\caption{\textbf{One-to-one mapping from CAST/CAST-conv+perm pipeline steps to the certificate theorems of Section~\mainref{Main_theory_gen}.}  Each row pairs an empirical knob in the implementation (left) with the specific theorem, lemma, or corollary that motivates it (right).  The pipeline is not a heuristic stack: every component is a certificate-improving move under the deterministic mask-certificate framework, with the source SER checkpoint chosen by the Marchenko--Pastur edge analysis, the per-layer pruning budget allocated by Corollary~\mainref{cor:mp_edge_data_path_budget}, the 2:4 mask selected by minimising a Lemma~\mainref{lem:deterministic_ce_path_bound}-inspired cost, the prune--restore step certificate-improving by Theorem~\mainref{thm:deterministic_prune_restore_certificate}, the \(\alpha_{\text{ser}}\) prior biasing toward the SER-validated kept set, and the Cin permutation alignment minimising the dispersion of \(B_T\) contributions across 4-tuples.}
...
SER \(s=0.35\) starting checkpoint
  & Marchenko--Pastur edge \(\sigma_+\) as bulk/spike threshold (Cor.~\mainref{cor:mp_edge_data_path_budget}); the bulk-only pruning rule chooses entries below \(\sigma_+\) and so is certificate-cheap by construction. \\
\addlinespace[2pt]
...
% =====================================================
\section{Migrated discussion: comparison with prior pruning methods on DeiT}
\label{sec:s37-deit-comparison}
% =====================================================

\noindent\emph{The following text is the original \mainpaper{}-\S3.7
head-to-head comparison against published DeiT pruning baselines (VTP,
PoWER-BERT, HVT, CP-ViT, the four head-to-head methods reported by
CP-ViT). The main manuscript records only the headline outcome; the
full per-method tables, the FLOP-vs-parameter accounting, and the
caveats about token-pruning vs weight-pruning protocol equivalence
are migrated here.}
