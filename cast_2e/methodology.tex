...
\item the reported hardware speedup measurements, including the A100 native-2:4 throughput data and the $6{:}12 \to 2{:}4$ projection benchmark used to translate non-deployable certificate-selected cells into deployable ones;
...
\path{cast_2e/code/benchmark_6_12_to_2_4_projection.py} & \S\ref{sec:speedup} \\
...
\begin{description}
\item[Practical (hardware) speedup.] The wall-clock images-per-second improvement on a specific GPU, kernel selection, and batch size. We use NVIDIA's PyTorch \path{torch.sparse.to_sparse_semi_structured} for the 2:4 path on A100 (sparse Tensor Core) and report the same measurement on L4. Other $k{:}n$ patterns do not have native kernels at present and are reported only as theoretical FLOP reductions.
...
The benchmark in \path{cast_2e/code/benchmark_sparse_throughput.py} converts each Linear layer with a 2:4 mask into a \path{torch.sparse.SparseSemiStructuredTensor} and runs warm-up + measurement passes at the configured batch size. The reported throughput in \path{cast_2e/benchmarks/} is the median of the last 100 iterations after 30 warm-up iterations.

\begin{table}[h]
\centering
\caption{A100 throughput (images/s) for the CAST configurations reported in the paper.}
\label{tab:a100-throughput}
\scriptsize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}Xrr>{\raggedright\arraybackslash}p{0.23\textwidth}}
\toprule
Configuration & Throughput (im/s) & Speedup vs.\ dense & Source \\
\midrule
Dense ViT-B/16 (batch 128) & 463.6 & --- & \path{vitb_bench.json} \\
2:4 ViT-B/16 (sparse Tensor Core) & 1254.1 & $2.705\times$ & \path{vitb_bench_with_24.json} \\
\midrule
Dense ViT-L/16 + ToMe $r=8$ & 1248.7 & $1.298\times$ & \path{vitl_canonical/results.json} \\
2:4 ViT-L/16 + ToMe $r=8$ (sparse) & 1489.9 & $1.549\times$ & \path{vitl_canonical/results.json} \\
\midrule
Dense ConvNeXtV2-B (batch 128) & 443.0 & --- & \path{convnextv2_canonical/results.json} \\
2:4 ConvNeXtV2-B (sparse Linear) & 481.2 & $1.086\times$ & \path{convnextv2_canonical/results.json} \\
\midrule
L4 ViT-B 2:4 + ToMe (CAST row) & 837 & $1.84\times$ & main FLOP table \\
\bottomrule
\end{tabularx}
\end{table}
...
\subsection{The \texorpdfstring{$6{:}12 \to 2{:}4$}{6:12 to 2:4} projection (deployable speedup)}
\label{subsec:6to24}

Several cells in the CAST-2E sweep produce non-deployable patterns ($6{:}12$, $8{:}16$, etc.) that are selected by the cert-inspired sweep. To translate these into deployable speedup numbers, \path{cast_2e/code/benchmark_6_12_to_2_4_projection.py} re-projects each $6{:}12$ block into the nearest $2{:}4$ block by enumerating the $6$ canonical $2{:}4$ patterns inside each $4$-sub-block of a $12$-tuple, choosing per-sub-block the pattern of lowest Hamming distance to the $6{:}12$ pattern. The resulting model is then benchmarked on the same A100 \path{torch.sparse.SparseSemiStructuredTensor} kernel as in \S\ref{subsec:a100-bench}.

The audit in \texttt{AUDIT.md} records that none of the $49$ tested ViT-B $6{:}12$ cells projects to a pure $2{:}4$ pattern in every $4$-sub-block; the resulting masks contain a mix of $0$, $2$, and $4$ nonzeros per sub-block. For deployable-speedup purposes \mainpaper{} therefore reports the $2{:}4$ speedup ($2.705\times$ on A100, $1.84\times$ on L4) as the deployable 2:4 number, and reports the $6{:}12$ speedup only as a theoretical FLOP reduction.

\subsection{ResNet and ConvNeXtV2 throughput}
The ResNet50/d/101d/152d post-FT models with $8{:}16$ structured sparsity have no native sparse kernel for general $k{:}n \neq 2{:}4$, so we report only the theoretical $50\%$ MAC reduction. The ConvNeXtV2-Base D816, D1216, and D48 cells are similarly reported only theoretically for non-$2{:}4$ patterns. The ConvNeXtV2-B canonical $2{:}4$ row, however, does run on the A100 sparse Linear kernel: at batch $128$ we measure $443.0$ images/s dense and $\mathbf{481.2}$ images/s sparse, a $\mathbf{1.086{\times}}$ kernel speedup (Table~\ref{tab:a100-throughput}). The ratio is intentionally smaller than the ViT-B/16 $2.705\times$ and the ViT-L/16 $1.549\times$: pointwise-conv $1{\times}1$ Linears (the only layers we sparsify in ConvNeXtV2 with the current kernel) are roughly $40\%$ of MAC budget, vs.\ $\sim 70\%$ for ViT MLP+QKV; the convolution-side $7{\times}7$ depthwise stems and patchify layers are not 2:4-eligible and bottleneck wall-clock. For deployable inference of the $k{:}n \neq 2{:}4$ ConvNeXtV2 cells we re-project to $2{:}4$ as in \S\ref{subsec:6to24}; this re-projection costs $\approx 0.2$ pp in post-FT top-1 (within the FT-noise band) at the gain of native A100 kernels.
...
CAST (\textbf{C}ertificate-\textbf{A}ware \textbf{S}parse-\textbf{T}oken conversion) is the procedure used to produce the second row of Table~\mainref{tab:param_to_flop_followup}.  This section records the algorithmic details, the connections to the deterministic certificate framework of Section~\mainref{Main_theory_gen}, and the role played by the RMT/SER pipeline that supplies CAST's input checkpoint.
...
ConvNeXtV2-Base & dense & \textbf{CAST 12:16 dense+perm} (25\% sparse) & 15.4G & 25\% & \(\mathbf{86.35}\)\textsuperscript{\S} \\
ConvNeXtV2-Base & dense & \textbf{CAST 8:16 dense+perm} (50\% sparse) & 15.4G & 50\% & \(\mathbf{85.85}\)\textsuperscript{\S} \\
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize \textsuperscript{\dag}ViT-L/16 reduction is estimated from the analytical 22.81\% ToMe + 50\% 2:4-Linear share. \textsuperscript{\ddag}\textbf{CAST-conv+perm} = CAST-conv with the importance-balanced Cin permutation alignment of Section~\ref{subsec:perm} (``flatten the layer''), motivated by the ResNet pre-FT collapse paragraph below.  CAST-conv+perm on ResNet152d achieves \(81.33\%\) post-FT on \texttt{resnet152d.ra2\_in1k} (dense baseline \(82.86\%\)), a \(-1.53\) pp drop at \(\sim\!50\%\) sparse-execution MAC reduction.  \textsuperscript{\S}The wider \(6{:}12\), \(8{:}16\), and \(12{:}16\) endpoints are theoretical sparse-execution reductions unless projected to native 2:4 kernels; their role here is to show the accuracy benefit of larger \(n\)-choice spaces under the same certificate-aware projection family.}
\end{table}

\paragraph{Hardware-executable acceleration over ToMe at the same accuracy.}
This row reports a measured \emph{wall-clock} speedup over the ToMe-r=8 dense-kernel execution path at essentially the same ImageNet top-1: from \(593\) to \(\mathbf{837}\) \textbf{images/s} on an L4 GPU at batch \(128\), a \(\mathbf{1.41\!\times}\) incremental speedup attributable to the 2:4 kernel switch with ToMe held constant on both endpoints.  The post-conversion top-1 is \(82.92\%\). For context, the ToMe-only ViT-B/16 (AugReg) reference numbers in \cite{bolya2023tome} Table 8 are \(83.94\%\) at \(r=8\) and \(82.86\%\) at \(r=12\); our magnitude-2:4+ToMe-r=8 baseline at \(82.92\%\) is \(1.02\) pp below the dense-kernel ToMe-r=8 reference, reflecting the additional accuracy cost of structurally projecting the Linear weights to a 2:4 pattern. The 2:4 projection step provides hardware-executable sparse acceleration over the ToMe-r=8 endpoint at this accuracy level.  Composed with the analytical \(\sim\!1.30\!\times\) ToMe-only MAC reduction over the dense ViT-B baseline, the implied total speedup over the original dense ViT-B is approximately \(1.84\!\times\), at \(2.19\) pp accuracy cost from the dense \(85.11\%\) checkpoint and \(1.63\) pp from the \(s=0.35\) Hybrid Magnitude--SER source checkpoint at \(84.55\%\).

\paragraph{A100 hardware-speedup measurements: total dense\(\to\)CAST speedups, all paths reported.}
...
\begin{table}[ht]
\centering
\caption{ViT-B/16 head-to-head reference at \(\sim 30\%\) compression on ImageNet-1k validation.  CP-ViT values are approximate because they are read from the published figure; the comparison is indicative because of the checkpoint mismatch (CP-ViT baseline \(77.91\%\) vs.\ our \(85.11\%\)).  Hybrid Magnitude--SER and the three within-paper baselines are reproduced from Table~\mainref{tab:vitb_current_methods}.  All entries are post-fine-tuning unless noted; CP-ViT used \(30\) FT epochs vs.\ our cumulative \(\sim 6\) FT epochs through \(s=0.30\).}
