...
The benchmark in \path{cast_2e/code/benchmark_deployable_backends.py} converts each compatible Linear layer with a 2:4 mask into a \path{torch.sparse.SparseSemiStructuredTensor} and runs warm-up + measurement passes at the configured batch size. For flattened-2:4 Conv2d checkpoints, it optionally lowers Conv2d to im2col+\(\mathrm{Linear}\), then applies the same native 2:4 Linear backend. The reported throughput is the median after warm-up. The table below reports only read-only measurements of existing checkpoint endpoints; no projection or fine-tuning is performed during this audit.
...
For ResNet, the audit checks two different Conv2d notions. The paper's MAC accounting flattens each Conv2d kernel to a matrix and verifies exact \(2{:}4\) groups in that flattened layout. TensorRT-style Conv2d sparsity is stricter: each group of four input channels must contain at most two nonzeros at every output channel and kernel pixel. The current ResNet50/50d/101d/152d CAST-conv+perm checkpoints pass the flattened MAC audit but fail the TensorRT Conv2d audit, with \(913{,}673\), \(904{,}895\), \(1{,}700{,}005\), and \(2{,}356{,}907\) bad TensorRT groups respectively. Consequently the ResNet deployability number in \mainpaper{} is the exact sparse-im2col backend speedup relative to dense im2col, not a native TensorRT Conv2d speedup.
...
\medskip
The largest measured endpoint speedup is the \(2.705\!\times\) ViT-B 2:4-alone result on A100. The same-checkpoint A40 ratios are lower but more directly tied to the validation rows in Table~\ref{tab:param_to_flop_followup}. They should be read as implementation-specific backend measurements, not theoretical limits.
