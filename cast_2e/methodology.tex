...
% Cross-paper reference: points to a label in the main paper.
\newcommand{\mainref}[1]{the main paper}
...
\subsection{Scope}
\mainpaper{} establishes a Marchenko--Pastur (MP)-driven theory of deterministic pruning certificates and reports results: the multi-architecture Hybrid Magnitude--SER table, the CAST 2:4 + ToMe accuracy/throughput row, and the deterministic-certificate audit. The main paper also contains the proofs.
...
\mainpaper{} alone is sufficient for: the theoretical statements, all proofs, the accuracy + speedup numbers (Table~2 and the CAST FLOP table), the theory-to-numerics mapping table, and the high-level reproducibility narrative.
...
Several cells in the CAST-2E sweep produce non-deployable patterns ($6{:}12$, $8{:}16$, etc.) that are nonetheless cert-optimal. To translate these into deployable speedup numbers, \path{cast_2e/code/benchmark_6_12_to_2_4_projection.py} re-projects each $6{:}12$ block into the nearest $2{:}4$ block by enumerating the $6$ canonical $2{:}4$ patterns inside each $4$-sub-block of a $12$-tuple, choosing per-sub-block the pattern of lowest Hamming distance to the $6{:}12$ pattern. The resulting model is then benchmarked on the same A100 \path{torch.sparse.SparseSemiStructuredTensor} kernel as in \S\ref{subsec:a100-bench}.

The audit in \texttt{AUDIT.md} records that none of the $49$ tested ViT-B $6{:}12$ cells projects to a pure $2{:}4$ pattern in every $4$-sub-block; the resulting masks contain a mix of $0$, $2$, and $4$ nonzeros per sub-block. For deployable-speedup purposes \mainpaper{} therefore reports the $2{:}4$ speedup ($2.705\times$ on A100, $1.84\times$ on L4) as the production-ready number, and reports the $6{:}12$ speedup only as a theoretical FLOP reduction.
...
\item \emph{For the ViT family, the SER source plus $\alpha_{\mathrm{ser}} = 0.5$ is the strongest sweep setting.} The $0.66$ pp gain of ViT-B SER+$\alpha=0.5$ over dense+$\alpha=0$ at $6{:}12$ is small pre-FT, but the post-FT gap is larger: the FT-recovered ViT-B SER+$\alpha=0.5$ scores $83.74\%$ post-FT, $+0.5$ pp above the dense+$\alpha=0$ FT.

\item \emph{ConvNeXtV2 prefers the dense source.} The depthwise structure of ConvNeXtV2 means the SER prior, which is built for dense projections, transfers poorly. The $12{:}16$ dense-source cell is the strongest pre-FT, and after FT it produces the $86.35\%$ row of \mainpaper{}-Table~CAST.
\end{enumerate}
...
The pattern observed: the favored modulation is asymmetric, with \(\beta_{\mathrm{attn}}=1.00\) (minimal spectral modulation for attention layers) and \(\beta_{\mathrm{mlp}}=2.00\) (strong spectral modulation for MLP layers).  This configuration achieves \(39.75\%\) top-1 at \(s=0.65\)---a gain of \(+6.67\) percentage points over the best uniform-\(\beta\) method (\(33.08\%\)) and \(+29.00\) pp over magnitude pruning (\(10.75\%\)).  The gain from layer-type differentiation is consistent across sparsities: \(+3.42\) pp at \(s=0.60\), \(+4.92\) pp at \(s=0.625\), and \(+5.92\) pp at \(s=0.675\).
...
This section gives the certification-audit numerics in a single self-contained reference. The audit verifies three theoretical bridges from \mainpaper{}-Section~5 against measured numerics on 18 trained models.
...
The audit supports the use of \mainpaper{}-Theorem~5.4 as a conservative certificate: across the audited cells there are zero violations of $|\Delta\,\mathrm{CE}| \le B_T$. The Bridge~1 correlations show that cells with smaller $B_T$ achieve higher post-projection top-1 inside an architecture. The Bridge~3 stratification indicates that the MP edge signal $\sigma_+$ is most informative for transformer architectures and least informative for ResNet/depthwise architectures --- consistent with the four-regime SEB analysis in \S\ref{sec:seb-protocols}.
...
Figure~\mainref{fig:param_vs_drop} is consistent with the random-matrix view of pruning that this paper builds on, but remains suggestive rather than confirmatory.  At \(s=0.50\) the fitted slope in \(\log_{10}\) parameter count is already negative (\(-0.73\) pp per decade, \(R=-0.31\)); by \(s=0.70\) it has steepened to \(-4.18\) pp per decade (\(R=-0.41\)).  The full 17-row plot has modest cross-architecture correlations, and the coarse pattern that larger models lose less under the same protocol has several possible non-RMT explanations: overparameterization, training-recipe differences, architecture-family effects, baseline accuracy heterogeneity, layer width, or general parameter-distribution effects.  The narrower claim is the \emph{systematic ordering}: the fitted slope becomes monotonically more negative as sparsity grows, matching the random-matrix prediction that each weight matrix decomposes approximately into a deterministic spike component carrying task signal and a Marchenko--Pastur-like bulk carrying random-matrix noise.
...
The predicted directional sign, larger model and smaller drop, is observed in every panel; the magnitude of the slope grows roughly in lockstep with sparsity.  Most checkpoints remain close to the dense baseline at moderate sparsity, and the large departures appear once the protocol begins to remove components close to or beyond the MP edge, where the classical RMT prediction becomes relevant.  Figure~\mainref{fig:param_vs_drop} is not direct confirmation of the bulk-vs-spike decomposition; direct confirmation requires per-layer measurements of the certificate quantities, reported in Section~\mainref{subsec:theory_numerics_alignment} below.  The figure is evidence consistent with the central premise of the paper: as networks grow, the bulk-vs-spike decomposition that motivates SER appears to become a better description of where the prunable capacity lives, so the same protocol becomes more accuracy-preserving at scale.
...
\begin{table}[ht]
\centering
\caption{\textbf{Mapping from CAST/CAST-conv+perm pipeline steps to the certificate theorems of Section~\mainref{Main_theory_gen}.}  Each row pairs an implementation component (left) with the theorem, lemma, or corollary that motivates it (right).  The source SER checkpoint is selected using the Marchenko--Pastur edge analysis; the per-layer pruning budget follows Corollary~\mainref{cor:mp_edge_data_path_budget}; the 2:4 mask minimises a Lemma~\mainref{lem:deterministic_ce_path_bound}-inspired cost; the prune--restore step follows Theorem~\mainref{thm:deterministic_prune_restore_certificate}; the \(\alpha_{\text{ser}}\) prior biases toward the SER-validated kept set; and the Cin permutation alignment reduces the dispersion of \(B_T\) contributions across 4-tuples.}
...
\(\alpha_{\text{ser}}\) Hamming prior
  & Theorem~\mainref{thm:deterministic_prune_restore_certificate} + an empirical Bayesian prior on the certificate-improving \(Q\): bias the 2:4 keep pattern toward the SER-validated unstructured mask, since SER kept entries are bulk-edge-validated. \\
\addlinespace[2pt]
Permutation alignment (CAST-conv+perm)
  & Cin importance balancing minimises the dispersion of \(B_T\) contributions across 4-tuples; the importance-balanced quartile interleave of Appendix~\mainref{app:perm} (``flatten the layer'') directly reduces the max-per-group certificate cost. \\
\bottomrule
\end{tabular}
\end{table}
