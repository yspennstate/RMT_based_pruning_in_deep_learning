...
> Release note (2026-05-11): this is a development ledger with pod-local and
> staging paths. The authoritative public-release mapping is
> `data/release_manifest.json`; use that manifest for Drive zip roots,
> checkpoint availability, and row-to-evidence lookup.

Rows below record the JSON evidence used during paper assembly. Public-release
users should treat this file as a development ledger; released Drive zip roots,
row evidence paths, and checkpoint availability are authoritative in
`data/release_manifest.json`.

## Main paper `tab:param_to_flop_followup` - FLOP / speedup rows
...
| ViT-B/16 CAST 6:12 SER+alpha=0.5 | 83.74% | `Downloads/cast_2e_resnet_review/github_archive/pod1_complete/vitb_ft_inline/D612_ser_a05/final_eval.json` (`post_ft_top1 = 0.83738`) | `student_final.pt` (Pod 1) | A100 |
...
## Main paper `tab:param_to_flop_followup` - speedup numbers

| Endpoint | Speedup | Evidence JSON |
|---|---|---|
| ViT-B/16 dense -> 2:4 alone, A100, batch 128 | 2.705x | `cast_2e/benchmarks/vitb_bench_with_24.json` (`kernel_speedup_2_4_vs_dense = 2.705253...`, dense 463.585 ips, sparse 1254.116 ips) |
| ViT-B/16 ToMe-only -> 2:4+ToMe, L4, batch 128 | 1.41x incremental | `cast_2e/sweep_results_initial/vitl_canonical/ft_phase/results.json` (dense 589.52 ips, sparse 836.26 ips, ratio 1.419) |
| ViT-L/16 ToMe-only -> 2:4+ToMe, A100, batch 128 | 1.193x incremental | `Downloads/cast_2e_resnet_review/github_archive/vitl_canonical/ft_phase/results.json` (dense 1248.66, sparse 1489.86) |

## Main paper `tab:multi_arch_hybrid` - Hybrid Mag-SER multi-arch
...
## Main paper `tab:result_comparison` - comparison vs prior work
...
| ELSA / LPViT / SERo | 50%-class DeiT-Base comparison row in each | various |
...
For a released row, use `data/release_manifest.json` first for Drive-relative
evidence paths and checkpoint availability. The row reproduction scripts live
under `cast_2e/code/`; the corresponding hyperparameters are summarized in main
paper `tab:reproduce_recipe`. Historical pod and local staging paths above are
retained only to document how the paper ledger was assembled.
