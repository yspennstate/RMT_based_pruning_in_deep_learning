...
This document defines the AWS experiment set, the rationale for each run, the
supporting theory, and the expected Table 5 reporting fields (ResNet rows +
appendix).

Prepared 2026-05-07 after AWS approved G/VT Spot quota (4 vCPUs, us-east-1).
...
These three runs produce the paper's Table 5 ResNet rows.

### Diagnostic ablations on resnet50 (3 runs, paper appendix)

These isolate which ingredient is responsible for each gain. Same SER source,
same FT schedule, same teacher — only the projection method varies.

| Run | Model | Mode | Purpose |
|---|---|---|---|
| A1 | resnet50 | cert_aware, **1×1-only**, free-restore | Clean 1×1 baseline (≈26% reduction). Shows the gap between the cleanest theory case and the full 1×1+3×3 case. |
| A2 | resnet50 | **magnitude**, 1×1+3×3, free-restore | Magnitude-only baseline without the certificate term; quantifies the cert-aware contribution. |
| A3 | resnet50 | cert_aware, 1×1+3×3, **no free-restore** | No-restoration baseline; quantifies the slot_values mechanism's contribution. |
...
parts of the pipeline have different levels of theory support. The categories
below separate formally justified components from implementation choices and
ablation baselines.

### Strong theory grounding (paper-aligned)

1. **SER s=0.35 source checkpoint.** Paper §3 / §5 — Sparsity-Equivalent Replacement is the RMT-based magnitude prior derived from the bulk eigenvalue distribution. Every CAST-2E run starts from a checkpoint that already passed this gate. **Used without modification.**
...
3. **Covariance-form certificate cost (`r^T C_g r`).** Includes the cross-terms
   ...
   correlated input channels (typical in deep ResNet stages), the
   cross-terms are not negligible and must be in the cost function. v3 implements
   this, replacing the v1/v2 diagonal approximation.
...
### Reasonable with caveat (paper-aligned with a footnote)
...
5. **3×3 conv via im2col reshape.** Reshape `[Cout, Cin, kH, kW] → [Cout, Cin·kH·kW]`,
   ...
   correspond to different `(kernel_position, channel)` pairs. This is a more
   ...
   **Suggested footnote:** *"For 3×3 convolutions we apply CAST via the im2col
...
6. **3-epoch distill fine-tune** with `α·KL(T) + (1−α)·CE` loss, dense teacher,
   ...
   theory does not prescribe FT schedules; this choice mirrors the original ViT
   experiments for consistency rather than as a theoretical consequence.

7. **Mask freezing during FT** — `freeze_grad_at_masked` + `apply_masks` after
   `optimizer.step()`. This is an implementation correctness requirement: without
   it, the projection collapses on the first gradient step.

### Ablation baseline

8. **Magnitude-only 2:4** (run A2). No certificate, just `argmax_2(|W|)` per
   4-tuple. This ablation quantifies the certificate contribution.
...
sparsity_after:
  ...
  bad_groups_after: 0  # required to be 0
eval:
  pre_ft_top1: float
  post_ft_top1: float
...
5. Re-evaluate before each next run: confirm spot instance remains active and S3 backup is current
...
- 6 manifest.yaml files in S3 → Table 5 ResNet rows and appendix material
...
4. **`post_ft_top1` is within ±0.5 pp of `dense_teacher_top1`** — the primary
   "maintained ImageNet top-1" comparison is against the **DENSE** baseline
   (the original timm-published score), NOT against the post-projection
...
Failures are logged but excluded from paper tables. If a run hits a spot
interruption that loses more than the last epoch boundary, it restarts
from the latest S3 checkpoint, not from scratch; this is recorded in the
manifest's `interruptions:` list for transparency.

## Excluded from this run

- ConvNeXt — not BasicBlock or bottleneck; the prior GCP attempt used a
  Linear-only pipeline, which was incompatible. With v3 code
  ...
  paper analysis. **Excluded from the current sweep.**
- Swin — windowed-shifted attention; excluded from the current sweep for the
  same reason.
- Hardware throughput speedup — PyTorch `to_sparse_semi_structured` is Linear-
  ...
  speedup unless each Conv2d is wrapped in `Unfold + Linear + Fold`. **Covered
  by a paper footnote; backend timing requires a separate deployment study.**
- BasicBlock ResNets (resnet18, resnet34) — 3×3 dominates more, but the paper
  story is bottleneck-focused. **Excluded from the current sweep.**
...
| 50% reduction missed (e.g. only 47%) | Low | catalog includes per-layer eligible_fraction to identify limiting layers |
...
The script handles: instance launch, setup.sh upload, S3 sync, the 6 runs
