...
def stage2_command(model_name_value: str, cache_dir: Path, run_dir: Path, args: argparse.Namespace) -> list[str]:
    return [
        sys.executable,
        "-u",
...
def run_exact_v8_stage_if_needed(
    ...
    subprocess.run(stage2_command(model_name_value, cache_dir, run_dir, args), cwd=OPTUNA_ROOT, check=True)
...
def main() -> None:
    ...
    run_exact_v8_stage_if_needed(args.model_name, cache_dir, run_dir, results_path, checkpoint_dir, args)
